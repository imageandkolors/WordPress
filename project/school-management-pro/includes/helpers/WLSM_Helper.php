...
class WLSM_Helper {
	...
	public static function medium_list($school_id = null) {
		global $wpdb;

		if ($school_id) {
			$mediums = $wpdb->get_results($wpdb->prepare('SELECT * FROM ' . WLSM_MEDIUM . ' WHERE school_id = %d', $school_id));
		} else {
	...
	public static function house_list($school_id = null) {
		global $wpdb;

		if ($school_id) {
			$houses = $wpdb->get_results($wpdb->prepare('SELECT * FROM ' . WLSM_HOUSE . ' WHERE school_id = %d', $school_id));
		} else {
			$houses = $wpdb->get_results('SELECT * FROM ' . WLSM_HOUSE);
		}

		$house_list = array();
		foreach ($houses as $house) {
			$house_list[$house->ID] = ucfirst($house->label);
		}
		return $house_list;
	}
	...
	public static function fee_type_list() {
		return array(
			'transport' => esc_html__( 'Transport', 'school-management' ),
			'admission' => esc_html__( 'Admission', 'school-management' ),
			'tuition'   => esc_html__( 'Tuition', 'school-management' ),
			'hostel'    => esc_html__( 'Hostel', 'school-management' ),
			'exam'      => esc_html__( 'Exam', 'school-management' ),
			'library'   => esc_html__( 'Library', 'school-management' ),
			'other'     => esc_html__( 'Other', 'school-management' ),
		);
	}

	public static function normalize_fee_type( $fee_type ) {
		$fee_type = maybe_unserialize( $fee_type );

		if ( is_array( $fee_type ) ) {
			$fee_type = reset( $fee_type );
		}

		$fee_type = sanitize_text_field( (string) $fee_type );
		$allowed  = array_keys( self::fee_type_list() );

		if ( in_array( $fee_type, $allowed, true ) ) {
			return $fee_type;
		}
	...
	public static function get_fee_type_text( $fee_type ) {
		$fee_type = self::normalize_fee_type( $fee_type );
		$types    = self::fee_type_list();

		if ( $fee_type && isset( $types[ $fee_type ] ) ) {
			return $types[ $fee_type ];
		}
	...
	public static function get_id_card_dynamic_fields() {
		return array(
			'school-logo' => array(
				'enable' => 1,
				'props'  => array(
	...
	public static function certificate_field_labels() {
		return array(
			...
			'serial-number'       => esc_html__( 'Serial NO.', 'school-management' ),
			...
			'phone'               => esc_html__( 'Phone', 'school-management' ),
			'address'             => esc_html__( 'Address', 'school-management' ),
			'signature'           => esc_html__( 'Signature', 'school-management' ),
