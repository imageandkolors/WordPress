...
class SM_Bigbluebutton_Api
{
	 ...
	 */
	public static function create_meeting($name, $moderator_code, $viewer_code, $recordable, $meeting_id,  $logout_url, $options = array())
	{
		...
		if (! empty($options) && is_array($options)) {
			$arr_params = array_merge($arr_params, $options);
		}


		$url = self::build_url('create', $arr_params);

		error_log('BBB Create Meeting URL: ' . $url);
	 ...
	 */
	public static function get_join_meeting_url($name, $moderator_code, $viewer_code, $recordable, $meeting_id, $logout_url, $mode, $options = array())
	{

		$uname = sanitize_text_field($name);

		if (! self::is_meeting_running($meeting_id)) {
			$code_error = self::create_meeting($name, $moderator_code, $viewer_code, $recordable, $meeting_id,  $logout_url, $options);
			if (200 !== $code_error) {
