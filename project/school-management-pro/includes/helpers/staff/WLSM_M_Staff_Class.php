...
class WLSM_M_Staff_Class
{
	...
	public static function get_exams($school_id, $class_id, $session_id = null) {
		global $wpdb;

		if ($session_id) {
			$session = $wpdb->get_row($wpdb->prepare("SELECT start_date, end_date FROM " . WLSM_SESSIONS . " WHERE ID = %d", $session_id));
		}

		$where = '';
		if (!empty($session)) {
			$where = " AND (ex.start_date BETWEEN '" . $session->start_date . "' AND '" . $session->end_date . "')";
		}

		$exams = $wpdb->get_results($wpdb->prepare(
			"SELECT ex.ID, ex.label, ex.exam_type
			...
			WHERE s.ID = %d AND cs.class_id = %d" . $where . "
	...
	public static function get_student_type_by_label($school_id, $label) {
		global $wpdb;
		$section = $wpdb->get_row($wpdb->prepare('SELECT me.ID FROM ' . WLSM_STUDENT_TYPE . ' as me WHERE me.school_id = %d AND me.label = %s', $school_id, $label));
		return $section;
	}
	...
	public static function get_medium_by_label( $school_id, $label) {
		global $wpdb;
		$medium = $wpdb->get_row($wpdb->prepare('SELECT me.ID, me.label FROM ' . WLSM_MEDIUM . ' as me WHERE me.school_id = %d AND me.label = %s', $school_id, $label));
		return $medium;
	}

	public static function get_house_page_url()
	{
		return admin_url('admin.php?page=' . WLSM_MENU_STAFF_HOUSE);
	}

	public static function fetch_house_query_count($school_id)
	{
		$query = 'SELECT COUNT(DISTINCT me.ID) FROM ' . WLSM_HOUSE . ' as me JOIN ' . WLSM_SCHOOLS . ' as s ON s.ID = me.school_id WHERE me.school_id = ' . absint($school_id);
		return $query;
	}

	public static function fetch_house_query_group_by()
	{
		$group_by = 'GROUP BY me.ID';
		return $group_by;
	}

	public static function fetch_house_query($school_id)
	{
		$query = 'SELECT me.ID, me.label FROM ' . WLSM_HOUSE . ' as me
		WHERE me.school_id = ' . absint($school_id);
		return $query;
	}

	public static function get_house($school_id, $id)
	{
		global $wpdb;
		$section = $wpdb->get_row($wpdb->prepare('SELECT me.ID FROM ' . WLSM_HOUSE . ' as me WHERE me.school_id = %d AND me.ID = %d', $school_id, $id));
		return $section;
	}

	public static function get_house_by_label($school_id, $label)
	{
		global $wpdb;
		$house = $wpdb->get_row($wpdb->prepare('SELECT me.ID, me.label FROM ' . WLSM_HOUSE . ' as me WHERE me.school_id = %d AND me.label = %s', $school_id, $label));
		return $house;
	}
	...
	public static function fetch_class_by_section_id($school_id, $section_id)
	{
		...
		$class = $wpdb->get_results($wpdb->prepare('SELECT DISTINCT(c.ID), c.label FROM ' . WLSM_CLASS_SCHOOL . ' as cs
			...
			WHERE cs.school_id = %d AND se.ID = %d ORDER BY c.ID ASC', $school_id, $section_id));
		return $class;
	}
	...
	public static function get_class_students($school_id, $session_id, $class_id, $only_active = true)
    {
        ...
        $students = $wpdb->get_results($wpdb->prepare('SELECT sr.ID, sr.name, sr.enrollment_number, sr.roll_number, sr.phone, sr.email, sr.father_name, sr.father_phone, c.label as class_label, se.label as section_label FROM ' . WLSM_STUDENT_RECORDS . ' as sr
            JOIN ' . WLSM_SECTIONS . ' as se ON se.ID = sr.section_id
	...
	{
		$query = 'SELECT n.ID, n.title, n.description, n.attachment, n.url, n.link_to, n.is_active, n.created_at, u.user_login as username FROM ' . WLSM_NOTICES . ' as n LEFT OUTER JOIN ' . WLSM_USERS . ' as u ON u.ID = n.added_by WHERE n.school_id = ' . absint($school_id);
		return $query;
	}
	...
	{
		$query = 'SELECT ev.ID, ev.title, ev.description, ev.event_date, ev.is_active, COUNT(sr.ID) as students_count FROM ' . WLSM_EVENTS . ' as ev
	...
	public static function get_chapters($subject_id, $school_id = '', $session_id = '') {
		global $wpdb;
		$query = 'SELECT cp.ID, cp.title, cp.subject_id FROM ' . WLSM_CHAPTER . ' as cp
		JOIN ' . WLSM_SUBJECTS . ' as sb ON sb.ID = cp.subject_id
		LEFT OUTER JOIN ' . WLSM_CLASS_SCHOOL . ' as cs ON cs.ID = sb.class_school_id
		WHERE cp.subject_id = ' . absint($subject_id);

		if ($school_id) {
			$query .= ' AND cs.school_id = ' . absint($school_id);
		}
		if ($session_id) {
			$query .= ' AND cp.session_id = ' . absint($session_id);
		}
	...
	public static function fetch_study_material_query_count_by_class_id($school_id, $class_id) {
		$query = 'SELECT COUNT(DISTINCT sm.ID) FROM ' . WLSM_STUDY_MATERIALS . ' as sm
		JOIN ' . WLSM_CLASS_SCHOOL_STUDY_MATERIAL . ' as ssm ON ssm.study_material_id = sm.ID
		JOIN ' . WLSM_SCHOOLS . ' as s ON s.ID = sm.school_id
		JOIN ' . WLSM_CLASS_SCHOOL . ' as cs ON cs.ID = ssm.class_school_id
	...
	public static function fetch_study_material_query_count_by_admin_id($school_id, $admin_id) {
		$query = 'SELECT COUNT(DISTINCT sm.ID) FROM ' . WLSM_STUDY_MATERIALS . ' as sm
		JOIN ' . WLSM_CLASS_SCHOOL_STUDY_MATERIAL . ' as ssm ON ssm.study_material_id = sm.ID
		JOIN ' . WLSM_SCHOOLS . ' as s ON s.ID = sm.school_id
		JOIN ' . WLSM_CLASS_SCHOOL . ' as cs ON cs.ID = ssm.class_school_id
	...
	public static function fetch_homework_submission($school_id, $session_id, $submission_id)
	{
		global $wpdb;
		$submission = $wpdb->get_row($wpdb->prepare('SELECT hs.ID, hs.description, hs.submission_id, hs.attachments, hs.created_at, hw.title, sr.roll_number, sr.name, c.label as class_label FROM ' . WLSM_HOMEWORK_SUBMISSION . ' as hs
		JOIN ' . WLSM_SCHOOLS . ' as s ON s.ID = hs.school_id
		JOIN ' . WLSM_SESSIONS . ' as ss ON ss.ID = hs.session_id
		JOIN ' . WLSM_HOMEWORK . ' as hw ON hw.ID = hs.submission_id
		LEFT OUTER JOIN ' . WLSM_HOMEWORK_SECTION . ' as hwse ON hwse.homework_id = hw.ID
		...
		LEFT OUTER JOIN ' . WLSM_CLASSES . ' as c ON c.ID = cs.class_id
		LEFT OUTER JOIN ' . WLSM_STUDENT_RECORDS . ' as sr ON sr.ID = student_id
		WHERE s.ID = %d AND hs.session_id = %d AND hs.ID = %d', $school_id, $session_id, $submission_id));
		return $submission;
	}
	...
	{
		global $wpdb;
		$homework = $wpdb->get_row($wpdb->prepare('SELECT hw.ID, hw.title, hw.subject, hw.description, hw.downloadable, hw.attachments, hw.attachment_url, hw.homework_date, hw.homework_due_date, c.ID as class_id, cs.ID as class_school_id, hwse.section_id, c.label as class_label, se.label as section_label, sj.label as subject_label FROM ' . WLSM_HOMEWORK . ' as hw
			JOIN ' . WLSM_SCHOOLS . ' as s ON s.ID = hw.school_id
			JOIN ' . WLSM_SESSIONS . ' as ss ON ss.ID = hw.session_id
			LEFT OUTER JOIN ' . WLSM_HOMEWORK_SECTION . ' as hwse ON hwse.homework_id = hw.ID
			LEFT OUTER JOIN ' . WLSM_SECTIONS . ' as se ON se.ID = hwse.section_id
			LEFT OUTER JOIN ' . WLSM_CLASS_SCHOOL . ' as cs ON cs.ID = se.class_school_id
			LEFT OUTER JOIN ' . WLSM_CLASSES . ' as c ON c.ID = cs.class_id
			LEFT OUTER JOIN ' . WLSM_SUBJECTS . ' as sj ON sj.ID = hw.subject
			WHERE s.ID = %d AND ss.ID = %d AND hw.ID = %d', $school_id, $session_id, $id));
		return $homework;
	}
	...
	{
		$query = 'SELECT se.ID, se.label as section_label, c.ID as class_id, c.label as class_label FROM ' . WLSM_ROUTINES . ' as rt
	...
	public static function fetch_time_table_by_class_id($school_id, $class_id) {
		 $query = 'SELECT se.ID, se.label AS section_label, c.ID as class_id, c.label AS class_label, sj.label AS subject_label, rt.room_number, rt.start_time, rt.end_time, rt.day
	...
	{
		$query = 'SELECT lv.ID, lv.description, lv.start_date, lv.end_date, lv.is_approved, lv.approved_by, c.ID as class_id, c.label as class_label, se.ID as section_id, se.label as section_label, sr.ID as student_id, sr.enrollment_number, sr.name as student_name FROM ' . WLSM_LEAVES . ' as lv
	...
	{
		if (1 == absint($is_approved)) {
			if ($color) {
				return '<span class="wlsm-font-bold wlsm-text-primary text-primary">' . self::get_approved_text() . '</span>';
			} else {
				return self::get_approved_text();
			}
		} elseif (2 == absint($is_approved)) {
			if ($color) {
				return '<span class="wlsm-font-bold wlsm-text-danger text-danger">' . self::get_rejected_text() . '</span>';
			} else {
				return self::get_rejected_text();
			}
		}

		if ($color) {
			return '<span class="wlsm-font-bold wlsm-text-warning text-warning">' . self::get_unapproved_text() . '</span>';
		} else {
	...
	{
		return esc_html__('Pending', 'school-management');
	}

	public static function get_rejected_text()
	{
		return esc_html__('Rejected', 'school-management');
	}
	...
	public static function get_holidays_page_url()
	{
		return admin_url('admin.php?page=' . WLSM_MENU_STAFF_HOLIDAYS);
	}

	public static function fetch_holidays_query($school_id)
	{
		$query = 'SELECT h.ID, h.description, h.start_date, h.end_date FROM ' . WLSM_HOLIDAYS . ' as h WHERE h.school_id = ' . absint($school_id);
		return $query;
	}

	public static function fetch_holidays_query_group_by()
	{
		$group_by = 'GROUP BY h.ID';
		return $group_by;
	}

	public static function fetch_holidays_query_count($school_id)
	{
		$query = 'SELECT COUNT(DISTINCT h.ID) FROM ' . WLSM_HOLIDAYS . ' as h WHERE h.school_id = ' . absint($school_id);
		return $query;
	}

	public static function get_holiday($school_id, $id)
	{
		global $wpdb;
		$holiday = $wpdb->get_row($wpdb->prepare('SELECT h.ID, h.description, h.start_date, h.end_date FROM ' . WLSM_HOLIDAYS . ' as h WHERE h.school_id = %d AND h.ID = %d', $school_id, $id));
		return $holiday;
	}

	public static function is_holiday($school_id, $date)
	{
		global $wpdb;
		$holiday = $wpdb->get_row($wpdb->prepare('SELECT h.ID, h.description FROM ' . WLSM_HOLIDAYS . ' as h WHERE h.school_id = %d AND %s BETWEEN h.start_date AND h.end_date', $school_id, $date));
		return $holiday;
	}
}
