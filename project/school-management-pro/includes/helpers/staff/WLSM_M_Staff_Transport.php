...
class WLSM_M_Staff_Transport {
	...
	public static function count_hostels( $school_id ) {
		global $wpdb;
		$hostels = $wpdb->get_var( $wpdb->prepare( 'SELECT COUNT(h.ID) FROM ' . WLSM_HOSTELS . ' as h WHERE h.school_id = %d', $school_id ) );
		return $hostels;
	}

	public static function count_rooms( $school_id ) {
		global $wpdb;
		$hostel_rooms = $wpdb->get_var( $wpdb->prepare( 'SELECT COUNT(r.ID) FROM ' . WLSM_ROOMS . ' as r JOIN ' . WLSM_HOSTELS . ' as h ON r.hostel_id = h.ID WHERE h.school_id = %d', $school_id ) );
		return $hostel_rooms;
	}

	public static function count_boys_hostels( $school_id ) {
		global $wpdb;
		$boys_hostel = $wpdb->get_var( $wpdb->prepare( 'SELECT COUNT(h.ID) FROM ' . WLSM_HOSTELS . ' as h WHERE h.school_id = %d AND h.hostel_type = %s', $school_id, 'boys' ) );
		return $boys_hostel;
	}

	public static function count_girls_hostels( $school_id ) {
		global $wpdb;
		$girls_hostel = $wpdb->get_var( $wpdb->prepare( 'SELECT COUNT(h.ID) FROM ' . WLSM_HOSTELS . ' as h WHERE h.school_id = %d AND h.hostel_type = %s', $school_id, 'girls' ) );
		return $girls_hostel;
	}
}
