...
class WLSM_M_Staff_Library {
	...
	public static function check_library_card_number_exists( $school_id, $session_id, $library_card_number ) {
		global $wpdb;
		$library_card_number_exists = $wpdb->get_row(
			$wpdb->prepare(
				...
				JOIN ' . WLSM_SECTIONS . ' as se ON se.ID = sr.section_id
				JOIN ' . WLSM_CLASS_SCHOOL . ' as cs ON cs.ID = se.class_school_id
				WHERE cs.school_id = %d AND ss.ID = %d AND lc.card_number = %s', $school_id, $session_id, $library_card_number
			)
		);
		return $library_card_number_exists;
	}

	public static function check_student_library_card_exists( $school_id, $session_id, $student_id ) {
		global $wpdb;
		...
		return $student_library_card_exists;
	}
}
