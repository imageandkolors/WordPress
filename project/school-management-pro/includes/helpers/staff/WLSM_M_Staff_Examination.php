...
class WLSM_M_Staff_Examination
{
	...
	public static function get_academic_multi_group_report_page_url()
	{
		return admin_url('admin.php?page=' . WLSM_MENU_STAFF_ACADEMIC_MULTI_GROUP_REPORT);
	}
	...
	{
		$query = 'SELECT ex.ID, ex.label as exam_title, ex.exam_center, ex.start_date, ex.end_date, ex.is_active, ex.admit_cards_published, ex.time_table_published, ex.results_published, c.ID as class_id, c.label as class_label, GROUP_CONCAT(DISTINCT CONCAT(\'<strong>-</strong> \', c.label) ORDER BY c.ID SEPARATOR \'<br>\') as class_names FROM ' . WLSM_EXAMS . ' as ex
	...
	public static function fetch_academic_report_query_count($school_id, $session_id = null)
	{
		$where_session = '';
		if ($session_id) {
			$where_session = ' AND ar.session_id = ' . absint($session_id);
		}
		$query = 'SELECT COUNT(DISTINCT ar.ID) FROM ' . WLSM_ACADEMIC_REPORTS . ' as ar
		WHERE ar.school_id = ' . absint($school_id) . $where_session;
		return $query;
	}
	...
	public static function get_exam_labels($exam_ids)
	{
		...
		$query = "SELECT label FROM " . WLSM_EXAMS . " WHERE ID IN ({$exam_ids_string})";
	...
	public static function fetch_exam_query_count_by_class_id($school_id, $class_id)
	{
		$query = 'SELECT COUNT(DISTINCT ex.ID) FROM ' . WLSM_EXAMS . ' as ex
		LEFT OUTER JOIN ' . WLSM_CLASS_SCHOOL_EXAM . ' as csex ON csex.exam_id = ex.ID
		LEFT OUTER JOIN ' . WLSM_CLASS_SCHOOL . ' as cs ON cs.ID = csex.class_school_id
		LEFT OUTER JOIN ' . WLSM_CLASSES . ' as c ON c.ID = cs.class_id
		WHERE ex.school_id = ' . absint($school_id) . ' AND c.ID = ' . absint($class_id);
		return $query;
	}
	...
	{
		global $wpdb;
		$exam = $wpdb->get_row($wpdb->prepare('SELECT ex.ID, ex.label as exam_title, ex.show_rank, ex.rank_criteria, ex.show_remark, ex.show_eremark, ex.psychomotor_analysis, ex.exam_center, ex.start_date, ex.end_date, ex.grade_criteria, ex.psychomotor, ex.is_active, ex.enable_room_numbers, ex.exam_group, ex.admit_cards_published, ex.time_table_published, ex.results_published, ex.show_in_assessment, ex.enable_total_marks, ex.results_obtained_marks, ex.teacher_signature, ex.exam_type, ex.show_overall_result_status, ex.show_subject_wise_result_status, ex.overall_result_status_text, ex.subject_result_status_text, ex.subject_pass_threshold, ex.overall_pass_threshold, ex.overall_pass_text, ex.overall_fail_text, ex.subject_pass_text, ex.subject_fail_text, ex.maximum_failed_subjects FROM ' . WLSM_EXAMS . ' as ex
		JOIN ' . WLSM_SCHOOLS . ' as s ON s.ID = ex.school_id
		WHERE ex.school_id = %d AND ex.ID = %d', $school_id, $id));
		return $exam;
	}
	...
	public static function get_academic_multi_group_report($school_id, $id)
	{
		global $wpdb;

		$query = $wpdb->prepare(
			"SELECT * FROM " . WLSM_ACADEMIC_MULTI_GROUP_REPORTS . " WHERE school_id = %d AND ID = %d",
			$school_id,
			$id
		);
    ...
    {
        global $wpdb;
        $exam = $wpdb->get_row($wpdb->prepare('SELECT eg.ID, eg.label, eg.is_active  FROM ' . WLSM_EXAMS_GROUP . ' as eg
	...
	public static function fetch_exam_groups_by_ids($school_id, $group_ids)
	{
		global $wpdb;
		...
		$order_by = implode(', ', $group_ids);

		$query = "SELECT eg.ID, eg.label FROM " . WLSM_EXAMS_GROUP . " AS eg
			WHERE eg.school_id = " . absint($school_id) . " AND eg.ID IN (" . implode(',', $group_ids) . ")
			ORDER BY FIELD(eg.ID, " . $order_by . ")";

		return $wpdb->get_results($query);
	}

	public static function get_exam_group_labels($school_id, $group_ids)
	{
		if (! is_array($group_ids) || ! count($group_ids)) {
			return '';
		}

		$groups = self::fetch_exam_groups_by_ids($school_id, $group_ids);
		...
		$labels = array_map(
			function ($group) {
				return WLSM_M_Class::get_label_text($group->label);
			},
			$groups
		);
    ...
    {
        global $wpdb;
        $admit_cards = $wpdb->get_results($wpdb->prepare('SELECT ac.ID, ac.exam_id,  ac.roll_number,  ac.student_record_id as student_id, sr.name, sr.phone, sr.email, sr.enrollment_number, sr.photo_id, sr.email,  c.ID as class_id, c.label as class_label, se.label as section_label, ss.label as session_label FROM ' . WLSM_ADMIT_CARDS . ' as ac
            JOIN ' . WLSM_EXAMS . ' as ex ON ex.ID = ac.exam_id
	...
	public static function fetch_academic_report_query($school_id, $session_id = null)
    {
        global $wpdb;

        $where_session = '';
        if ($session_id) {
            $where_session = $wpdb->prepare(' AND ar.session_id = %d', $session_id);
        }

		$query = $wpdb->prepare(
			"SELECT ar.ID, ar.label AS report_label, c.ID AS class_id, c.label AS class_label, ar.exams AS exam_ids, eg.ID as group_id, eg.label AS group_label
			...
			WHERE ar.school_id = %d" . $where_session,
			$school_id
		);
	...
	public static function fetch_academic_report_query_by_class_id($school_id, $class_id, $session_id = null)
    {
        global $wpdb;
		...
		$query = $wpdb->prepare(
			...
			INNER JOIN " . WLSM_CLASSES . " AS c ON ar.class_id = c.ID
			INNER JOIN " . WLSM_EXAMS_GROUP . " AS eg ON ar.exam_group = eg.ID
			WHERE ar.school_id = %d AND c.ID = %d" . $where_session,
			$school_id,
			$class_id
		);
	...
	public static function fetch_academic_report_query_count_by_class_id($school_id, $class_id, $session_id = null)
	{
		$where_session = '';
		if ($session_id) {
			$where_session = ' AND ar.session_id = ' . absint($session_id);
		...
		$query = 'SELECT COUNT(DISTINCT ar.ID) FROM ' . WLSM_ACADEMIC_REPORTS . ' as ar
		INNER JOIN ' . WLSM_CLASSES . ' as c ON ar.class_id = c.ID
		WHERE ar.school_id = ' . absint($school_id) . ' AND c.ID = ' . absint($class_id) . $where_session;
		return $query;
	}

	public static function fetch_academic_multi_group_report_query($school_id, $session_id = 0)
	{
		global $wpdb;

		$query = $wpdb->prepare(
			"SELECT amgr.ID, amgr.label AS report_label, c.label AS class_label, s.label AS session_label, amgr.exam_groups AS group_ids
			...
			WHERE amgr.school_id = %d",
			$school_id
		);

		if ($session_id) {
			$query .= $wpdb->prepare(" AND amgr.session_id = %d", $session_id);
		}

		return $query;
	}

	public static function fetch_academic_multi_group_report_query_by_class_id($school_id, $class_id, $session_id = 0)
	{
		global $wpdb;
		...
		$query = $wpdb->prepare(
			...
			INNER JOIN " . WLSM_CLASSES . " AS c ON amgr.class_id = c.ID
			INNER JOIN " . WLSM_SESSIONS . " AS s ON amgr.session_id = s.ID
			WHERE amgr.school_id = %d AND c.ID = %d",
			$school_id,
			$class_id
		);
	...
	public static function fetch_academic_multi_group_report_query_group_by()
	{
		return 'GROUP BY amgr.ID';
	}

	public static function fetch_academic_multi_group_report_query_count($school_id, $session_id = 0)
	{
		global $wpdb;

		$query = $wpdb->prepare(
			"SELECT COUNT(DISTINCT amgr.ID)
			FROM " . WLSM_ACADEMIC_MULTI_GROUP_REPORTS . " AS amgr
			WHERE amgr.school_id = %d",
			$school_id
		);
	...
	{
		global $wpdb;
		$admit_card = $wpdb->get_row($wpdb->prepare('SELECT ac.ID, ac.exam_id, ac.roll_number, ac.student_record_id as student_id, sr.name, sr.phone, sr.enrollment_number, sr.admission_number, sr.photo_id, sr.email, c.label as class_label, se.label as section_label, sr.section_id, ss.label as session_label, sr.father_name FROM ' . WLSM_ADMIT_CARDS . ' as ac
			JOIN ' . WLSM_EXAMS . ' as ex ON ex.ID = ac.exam_id
			JOIN ' . WLSM_STUDENT_RECORDS . ' as sr ON sr.ID = ac.student_record_id
			JOIN ' . WLSM_SECTIONS . ' as se ON se.ID = sr.section_id
			JOIN ' . WLSM_CLASS_SCHOOL . ' as cs ON cs.ID = se.class_school_id
	...
	{
		global $wpdb;
		$exams = $wpdb->get_results($wpdb->prepare('SELECT DISTINCT ex.ID, ex.label as exam_title, ex.show_rank, ex.rank_criteria, ex.show_remark, ex.psychomotor_analysis, ex.psychomotor, ex.start_date, ex.end_date, ex.results_obtained_marks , ex.enable_total_marks, ex.teacher_signature FROM ' . WLSM_EXAMS . ' as ex
		JOIN ' . WLSM_CLASS_SCHOOL_EXAM . ' as csex ON csex.exam_id = ex.ID
		JOIN ' . WLSM_SCHOOLS . ' as s ON s.ID = ex.school_id
	...
	public static function get_class_school_exams_academic_report($school_id, $class_school_id, $academic_report_id)
	{
		...
		$exams = $wpdb->get_results(
			$wpdb->prepare(
				"SELECT DISTINCT ex.ID, ex.label AS exam_title, ex.exam_type, ex.show_rank, ex.rank_criteria, ex.show_remark, ex.psychomotor_analysis, ex.psychomotor, ex.start_date, ex.end_date, ex.results_obtained_marks, ex.enable_total_marks, ex.teacher_signature, ex.grade_criteria, ex.show_overall_result_status, ex.show_subject_wise_result_status, ex.overall_result_status_text, ex.subject_result_status_text, ex.subject_pass_threshold, ex.overall_pass_threshold, ex.overall_pass_text, ex.overall_fail_text, ex.subject_pass_text, ex.subject_fail_text, ex.maximum_failed_subjects, eg.ID as exam_group
				FROM " . WLSM_EXAMS . " AS ex
				LEFT JOIN " . WLSM_EXAMS_GROUP . " AS eg ON eg.ID = ex.exam_group
				JOIN " . WLSM_CLASS_SCHOOL_EXAM . " AS csex ON csex.exam_id = ex.ID
				JOIN " . WLSM_SCHOOLS . " AS s ON s.ID = ex.school_id
		...
		if (count($exams) > 1) {
			$first_exam_type = $exams[0]->exam_type;
			foreach ($exams as $exam) {
				if ($exam->exam_type !== $first_exam_type) {
					wp_send_json_error(esc_html__('Exams with different Exam Types cannot be combined in a single report.', 'school-management'));
				}
			}
		}
	...
	public static function get_class_school_exams_academic_multi_group_report($school_id, $class_school_id, $report_id)
	{
		global $wpdb;

		$report = self::get_academic_multi_group_report($school_id, $report_id);

		if (! $report) {
			return array();
		}

		$group_ids = json_decode($report->exam_groups, true);
		...
		$placeholders = implode(', ', array_fill(0, count($group_ids), '%d'));
		$order_field = implode(', ', $group_ids);

		$query_where = "WHERE ex.school_id = %d
				AND ex.is_active = 1
				AND csex.class_school_id = %d
				AND ex.exam_group IN (" . $placeholders . ")";

		if ( 'grade_only' === $report->exam_type ) {
			$query_where .= " AND ex.exam_type = 'grade_only'";
		} elseif ( 'marks_grades' === $report->exam_type ) {
			$query_where .= " AND ( ex.exam_type = 'marks_grades' OR ex.exam_type IS NULL )";
		}

		$query = $wpdb->prepare(
			"SELECT DISTINCT ex.ID,
				ex.label AS exam_title,
				ex.exam_type,
				ex.show_rank,
				ex.rank_criteria,
				ex.show_remark,
				ex.psychomotor_analysis,
				ex.psychomotor,
				ex.start_date,
				ex.end_date,
				ex.results_obtained_marks,
				ex.enable_total_marks,
				ex.teacher_signature,
				ex.grade_criteria,
				ex.show_overall_result_status,
				ex.show_subject_wise_result_status,
				ex.overall_result_status_text,
				ex.subject_result_status_text,
				ex.subject_pass_threshold,
				ex.overall_pass_threshold,
				ex.maximum_failed_subjects,
				ex.overall_pass_text,
				ex.overall_fail_text,
				ex.subject_pass_text,
				ex.subject_fail_text,
				ex.exam_group AS exam_group_id,
				eg.label AS exam_group_label
			FROM " . WLSM_EXAMS . " AS ex
			JOIN " . WLSM_CLASS_SCHOOL_EXAM . " AS csex ON csex.exam_id = ex.ID
			JOIN " . WLSM_SCHOOLS . " AS s ON s.ID = ex.school_id
			JOIN " . WLSM_EXAMS_GROUP . " AS eg ON eg.ID = ex.exam_group
			" . $query_where . "
			ORDER BY FIELD(ex.exam_group, " . $order_field . "), ex.start_date ASC",
			array_merge(array($school_id, $class_school_id), $group_ids)
		);
	...
	public static function get_exam_result_by_subject_code($school_id, $exam_id, $student_id, $subject_code)
	{
		...
		$exam_result = $wpdb->get_row(
			$wpdb->prepare(
				'SELECT ep.maximum_marks, er.obtained_marks, er.obtained_grade, er.remark, er.scale FROM ' . WLSM_EXAM_RESULTS . ' as er
			JOIN ' . WLSM_EXAM_PAPERS . ' as ep ON ep.ID = er.exam_paper_id
			JOIN ' . WLSM_EXAMS . ' as ex ON ex.ID = ep.exam_id
			JOIN ' . WLSM_ADMIT_CARDS . ' as ac ON ac.ID = er.admit_card_id
			JOIN ' . WLSM_STUDENT_RECORDS . ' as sr ON sr.ID = ac.student_record_id
			...
			JOIN ' . WLSM_SECTIONS . ' as se ON se.ID = sr.section_id
			JOIN ' . WLSM_CLASS_SCHOOL . ' as cs ON cs.ID = se.class_school_id
			LEFT JOIN ' . WLSM_SUBJECTS . ' as subj ON subj.class_school_id = cs.ID AND subj.code = %s
			WHERE ex.school_id = %d AND ep.exam_id = %d AND sr.ID = %d
				AND (
					ep.paper_code = %s
					OR ( ep.subject_id IS NOT NULL AND subj.ID IS NOT NULL AND ep.subject_id = subj.ID )
				)',
				$subject_code,
				$school_id,
				$exam_id,
		...
		if (! $exam_result) {
			error_log(
				sprintf(
					'WLSM: Exam result not found for school_id %d, exam_id %d, student_record_id %d, subject_code %s.',
					$school_id,
					$exam_id,
					$student_id,
					$subject_code
	...
	public static function get_exam_results($school_id, $exam_id, $order = 'ASC', $section_id = null)
    {
        global $wpdb;

        $where_section = '';
        if ($section_id) {
            $where_section = $wpdb->prepare(' AND sr.section_id = %d', $section_id);
        }

        $exam_results = $wpdb->get_results($wpdb->prepare('SELECT ac.ID, ac.ID as admit_card_id, ep.exam_id, ac.roll_number, c.label as class_label, se.label as section_label, sr.section_id, sr.ID as student_id, sr.name, sr.father_name, sr.phone, sr.enrollment_number, sr.admission_number, COALESCE(SUM(ep.maximum_marks), 0) as total_marks, COALESCE(SUM(er.obtained_marks), 0) as obtained_marks FROM ' . WLSM_EXAM_RESULTS . ' as er
            JOIN ' . WLSM_EXAM_PAPERS . ' as ep ON ep.ID = er.exam_paper_id
            JOIN ' . WLSM_EXAMS . ' as ex ON ex.ID = ep.exam_id
            JOIN ' . WLSM_ADMIT_CARDS . ' as ac ON ac.ID = er.admit_card_id
            JOIN ' . WLSM_STUDENT_RECORDS . ' as sr ON sr.ID = ac.student_record_id
            ...
            WHERE ex.school_id = %d AND ep.exam_id = %d' . $where_section . ' GROUP BY ac.ID', $school_id, $exam_id));
        return $exam_results;
    }
	...
	{
		global $wpdb;
		$exam_results = $wpdb->get_results($wpdb->prepare('SELECT er.exam_paper_id, er.ID, er.obtained_marks, er.obtained_grade, er.remark, er.teacher_remark, er.school_remark, er.scale, er.answer_key  FROM ' . WLSM_EXAM_RESULTS . ' as er
			JOIN ' . WLSM_ADMIT_CARDS . ' as ac ON ac.ID = er.admit_card_id
			JOIN ' . WLSM_STUDENT_RECORDS . ' as sr ON sr.ID = ac.student_record_id
			JOIN ' . WLSM_SECTIONS . ' as se ON se.ID = sr.section_id
			JOIN ' . WLSM_CLASS_SCHOOL . ' as cs ON cs.ID = se.class_school_id
	...
	public static function calculate_exam_ranks($school_id, $exam_id, $exam_results = array(), $admit_card_id = '', $show_total = false, $section_id = null)
	{
		if ($school_id && $exam_id) {
			$exam_results = WLSM_M_Staff_Examination::get_exam_results($school_id, $exam_id, 'DESC', $section_id);
		}
		...
		$total_students = count($student_ranks);

		if ($admit_card_id) {
			$rank = isset($student_ranks[$admit_card_id]) ? $student_ranks[$admit_card_id] : '-';
			if ($show_total && $rank !== '-') {
	...
	public static function get_exams_by_group_id($school_id, $group_id)
	{
		global $wpdb;
		$exams = $wpdb->get_results($wpdb->prepare('SELECT ID, label FROM ' . WLSM_EXAMS . ' WHERE school_id = %d AND exam_group = %d', $school_id, $group_id));
		return $exams;
	}
}
