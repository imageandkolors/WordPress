...
class WLSM_M_Setting
{
	...
	public static function get_settings_exam($school_id)
	{
		global $wpdb;
		$hide_subject_type = 0;
		$hide_subject_code = 0;

		$settings = $wpdb->get_row($wpdb->prepare('SELECT ID, setting_value FROM ' . WLSM_SETTINGS . ' WHERE school_id = %d AND setting_key = "exam"', $school_id));
		if ($settings) {
			$settings          = unserialize($settings->setting_value);
			$hide_subject_type = isset($settings['hide_subject_type']) ? (bool) $settings['hide_subject_type'] : 0;
			$hide_subject_code = isset($settings['hide_subject_code']) ? (bool) $settings['hide_subject_code'] : 0;
		}

		return array(
			'hide_subject_type' => $hide_subject_type,
			'hide_subject_code' => $hide_subject_code,
		);
	}
	...
	public static function get_settings_email_exam_result_report($school_id)
	{
		global $wpdb;
		...
		$settings = $wpdb->get_row($wpdb->prepare('SELECT ID, setting_value FROM ' . WLSM_SETTINGS . ' WHERE school_id = %d AND setting_key = "email_exam_result_report"', $school_id));
	...
	public static function get_settings_registration($school_id)
	{
		...
		if ($settings) {
			...
			$guardian_login    = isset($settings['guardian_login']) ? $settings['guardian_login'] : '';
			...
			$house             = isset($settings['house']) ? $settings['house'] : '';
		...
		return array(
			...
			'guardian_login'        => (bool)$guardian_login,
	...
	public static function get_settings_dashboard($school_id)
	{
		...
		if ($settings) {
			...
			$school_calendar          = isset($settings['school_calendar']) ? $settings['school_calendar'] : '';
			...
			$school_lessons           = isset($settings['school_lessons']) ? $settings['school_lessons'] : '';
		...
		return array(
			...
			'school_calendar'          => (bool)$school_calendar,
			...
			'school_lessons'           => (bool)$school_lessons,
	...
	public static function get_settings_sms_student_admission($school_id)
	{
		...
		if ($settings) {
			...
			$template_id = isset($settings['template_id']) ? $settings['template_id'] : '';
			$to_student = isset($settings['to_student']) ? (bool) $settings['to_student'] : true;
			$to_father  = isset($settings['to_father']) ? (bool) $settings['to_father'] : true;
			$to_mother  = isset($settings['to_mother']) ? (bool) $settings['to_mother'] : true;
		} else {
			$to_student = true;
			$to_father  = true;
			$to_mother  = true;
	...
	public static function get_settings_sms_exam_result_report($school_id)
	{
		...
		if ($settings) {
			...
			$message  = isset($settings['message']) ? $settings['message'] : '';
			$template_id = isset($settings['template_id']) ? $settings['template_id'] : '';
			$to_student = isset($settings['to_student']) ? (bool) $settings['to_student'] : false;
			$to_father  = isset($settings['to_father']) ? (bool) $settings['to_father'] : false;
			$to_mother  = isset($settings['to_mother']) ? (bool) $settings['to_mother'] : false;
		} else {
			$to_student = true;
			$to_father  = true;
			$to_mother  = true;
	...
	public static function get_settings_sms_academic_report($school_id)
	{
		global $wpdb;
		...
		$settings = $wpdb->get_row($wpdb->prepare('SELECT ID, setting_value FROM ' . WLSM_SETTINGS . ' WHERE school_id = %d AND setting_key = "sms_academic_report"', $school_id));

		if ($settings) {
			$settings = unserialize($settings->setting_value);
			$enable      = isset($settings['enable']) ? (bool) $settings['enable'] : false;
			$to_student  = isset($settings['to_student']) ? (bool) $settings['to_student'] : 0;
			$to_father   = isset($settings['to_father']) ? (bool) $settings['to_father'] : 0;
			$to_mother   = isset($settings['to_mother']) ? (bool) $settings['to_mother'] : 0;
			$message     = isset($settings['message']) ? $settings['message'] : '';
			$template_id = isset($settings['template_id']) ? $settings['template_id'] : '';
		}
	...
	public static function get_settings_sms_academic_multi_group_report($school_id)
	{
		global $wpdb;
		...
		$settings = $wpdb->get_row($wpdb->prepare('SELECT ID, setting_value FROM ' . WLSM_SETTINGS . ' WHERE school_id = %d AND setting_key = "sms_academic_multi_group_report"', $school_id));
	...
	public static function get_settings_africastalking($school_id)
	{
		global $wpdb;
		...
		$settings = $wpdb->get_row($wpdb->prepare('SELECT ID, setting_value FROM ' . WLSM_SETTINGS . ' WHERE school_id = %d AND setting_key = "africastalking"', $school_id));
		if ($settings) {
			$settings  = unserialize($settings->setting_value);
			$username  = isset($settings['username']) ? $settings['username'] : '';
			$api_key   = isset($settings['api_key']) ? $settings['api_key'] : '';
