		...
		<form action="<?php echo esc_url( admin_url( 'admin-ajax.php' ) ); ?>" method="post" id="wlsm-save-staff-form" enctype="multipart/form-data">
			...
			<div class="wlsm-form-section">
				...
				<div class="form-row" id="wlsm_permissions_container">
					...
					$permission_descriptions = WLSM_M_Role::get_permission_descriptions();
									...
									<div class="p-2">
										<div class="row">
											...
											foreach ( $permissions_in_group as $key => $value ) {
												$description = isset( $permission_descriptions[ $key ] ) ? $permission_descriptions[ $key ] : '';
												?>
												<div class="col-12 mb-1 wlsm-permission-item">
													<div class="form-check pl-0">
														<input <?php if ( ( WLSM_M_Role::get_admin_key() === $role ) || ( in_array( $key, $permissions ) && in_array( $key ,$staff_role_permissions ) ) ) { echo 'disabled'; } ?> <?php checked( in_array( $key, $permissions ), true, true ); ?> class="form-check-input mt-1 wlsm-group-permission wlsm-group-<?php echo esc_attr( $group_key ); ?>" type="checkbox" name="permission[]" id="wlsm_staff_permission_<?php echo esc_attr( $key ); ?>" value="<?php echo esc_attr( $key ); ?>" data-group="<?php echo esc_attr( $group_key ); ?>" style="position: relative; margin-left: 0;">
														<label class="form-check-label wlsm-font-bold ml-2" for="wlsm_staff_permission_<?php echo esc_attr( $key ); ?>" style="font-size: 0.9rem; line-height: 1.5;" <?php if ( $description ) { echo 'data-toggle="tooltip" title="' . esc_attr( $description ) . '"'; } ?>>
															<?php echo esc_html( $value ); ?>
															<?php if ( $description ) { ?>
