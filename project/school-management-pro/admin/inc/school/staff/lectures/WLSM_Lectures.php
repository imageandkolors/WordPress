...
class WLSM_Lecture {
	...
	public static function fetch_lecture() {
		$current_user = WLSM_M_Role::can('view_lessons');
		...
		$query = WLSM_M_Staff_Lecture::fetch_lecture_query($school_id, $session_id);
		...
		// Total query.
		$rows_query = WLSM_M_Staff_Lecture::fetch_lecture_query_count($school_id, $session_id);
	...
	public static function save_lecture() {
		...
		try {
			...
			if ($lecture_id) {
				$current_user = WLSM_M_Role::can('edit_lessons');
			} else {
				$current_user = WLSM_M_Role::can('add_lessons');
			}
	...
	public static function delete_lecture() {
		$current_user = WLSM_M_Role::can('delete_lessons');
	...
	public static function save_chapter() {
		try {
			...
			if (isset($_POST['classes'])) {
				$class_id = absint($_POST['classes']);
			} else {
				$class_id = isset( $_POST['class_id'] ) ? absint( $_POST['class_id'] ) : 0;
			}

			if (isset($_POST['subject'])) {
				$subject_id = absint($_POST['subject']);
			} else {
			...
			if ($chapter_id) {
				$current_user = WLSM_M_Role::can('edit_chapters');
			} else {
				$current_user = WLSM_M_Role::can('add_chapters');
			}
	...
	public static function fetch_chapter() {
		$current_user = WLSM_M_Role::can('view_chapters');
		...
		$query = WLSM_M_Staff_Lecture::fetch_chapter_query($school_id, $session_id);
		...
		// Total query.
		$rows_query = WLSM_M_Staff_Lecture::fetch_chapter_query_count($school_id, $session_id);
	...
	public static function delete_chapter() {
		$current_user = WLSM_M_Role::can('delete_chapters');
