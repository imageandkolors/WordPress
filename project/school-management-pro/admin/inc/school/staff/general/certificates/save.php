...
if ( isset( $_GET['id'] ) && ! empty( $_GET['id'] ) ) {
	...
	if ( $certificate ) {
		...
		$certificate_number_base   = $certificate->certificate_number_base;
		$certificate_number_padding = isset($certificate->certificate_number_padding) ? absint($certificate->certificate_number_padding) : 0;

		if ($certificate_number_padding > 0) {
			$certificate_number_base = str_pad($certificate_number_base, $certificate_number_padding, '0', STR_PAD_LEFT);
		}
		...
		<form action="<?php echo esc_url( admin_url( 'admin-ajax.php' ) ); ?>" method="post" id="wlsm-save-certificate-form">
			...
			<div class="wlsm-form-section">
				<div class="form-row justify-content-md-center">
					...
					<div class="form-group col-md-4">
						<label for="wlsm_orientation" class="wlsm-font-bold">
							<?php esc_html_e( 'Orientation', 'school-management' ); ?>:
						</label>
						<select name="orientation" class="form-control selectpicker" id="wlsm_orientation">
							<option value="portrait" <?php selected($orientation, 'portrait'); ?>><?php esc_html_e( 'Portrait', 'school-management' ); ?></option>
							<option value="landscape" <?php selected($orientation, 'landscape'); ?>><?php esc_html_e( 'Landscape', 'school-management' ); ?></option>
						</select>
					</div>

					<div class="form-group col-md-4">
						<label for="wlsm_certificate_number_prefix" class="wlsm-font-bold">
							<?php esc_html_e( 'Certificate Number Prefix', 'school-management' ); ?>:
						</label>
						<input type="text" name="certificate_number_prefix" class="form-control" id="wlsm_certificate_number_prefix" placeholder="<?php esc_attr_e( 'Enter prefix', 'school-management' ); ?>" value="<?php echo esc_attr( $certificate_number_prefix ); ?>">
					</div>

					<div class="form-group col-md-4">
						<label for="wlsm_certificate_number_base" class="wlsm-font-bold">
							<?php esc_html_e( 'Starting Base Number', 'school-management' ); ?>:
						</label>
						<input type="text" name="certificate_number_base" class="form-control" id="wlsm_certificate_number_base" placeholder="<?php esc_attr_e( 'Enter base number', 'school-management' ); ?>" value="<?php echo esc_attr( $certificate_number_base ); ?>">
					</div>
