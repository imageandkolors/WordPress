...
class WLSM_Staff_General
{
	public static function get_class_sections()
	{
		...
		try {
			...
			$fees = WLSM_M_Staff_Accountant::fetch_fees_by_class_student_type($school_id, $class_id, $student_type, $session_id);
			...
			if ($user_can_manage_subjects && ! $user_can_manage_class) {
				...
				$admin = WLSM_M_Role::get_user_admin($school_id, $user_id);
				if ($admin) {
					$subject = WLSM_Helper::getAssignedSubjects($admin->ID);
				} else {
					$subject = WLSM_M_Staff_Class::fetch_subject_query_by_class_id($school_id, $class_id);
				}
			} else {
	...
	public static function get_class_subjects()
	{
		...
		try {
			...
			if ($user_can_manage_subjects && ! $user_can_manage_class) {
				...
				$admin = WLSM_M_Role::get_user_admin($school_id, $user_id);
				if ($admin) {
					$subjects = WLSM_Helper::getAssignedSubjects($admin->ID);
				} else {
					$subjects = WLSM_M_Staff_Class::get_class_subjects($school_id, $class_id);
				}
			} else {
	...
	public static function get_class_chapter()
	{
		...
		try {
			...
			$chapters = WLSM_M_Staff_Class::get_chapters($subject_id, $school_id, $session_id);
	...
	public static function get_class_exams()
	{
		...
		try {
			...
			$exams = WLSM_M_Staff_Class::get_exams($school_id, $class_id, $session_id);
	...
	public static function save_student($current_user)
	{
		...
		try {
			...
			$house           = isset($_POST['house']) ? sanitize_text_field($_POST['house']) : '';
			...
			// Guardian Login Detail.
			$guardian_new_or_existing   = isset($_POST['guardian_new_or_existing']) ? sanitize_text_field($_POST['guardian_new_or_existing']) : '';
			$guardian_existing_username = isset($_POST['guardian_existing_username']) ? sanitize_text_field($_POST['guardian_existing_username']) : '';
			$guardian_new_login_email   = isset($_POST['guardian_new_login_email']) ? sanitize_text_field($_POST['guardian_new_login_email']) : '';
			$guardian_new_password      = isset($_POST['guardian_new_password']) ? sanitize_text_field($_POST['guardian_new_password']) : '';
			$guardian_username          = isset($_POST['guardian_username']) ? sanitize_text_field($_POST['guardian_username']) : '';
			$guardian_login_email       = isset($_POST['guardian_login_email']) ? sanitize_text_field($_POST['guardian_login_email']) : '';
			$guardian_password          = isset($_POST['guardian_password']) ? $_POST['guardian_password'] : '';
			...
			// For existing students, check if we need to clean up old subject assignments
			$need_subject_cleanup = false;
			if ($student_id && !empty($subjects_ids)) {
				// Check if class has changed or subjects have changed
				$current_class_id = isset($_POST['class_id']) ? absint($_POST['class_id']) : 0;
				if ($current_class_id && $current_class_id != $student->class_id) {
					$need_subject_cleanup = true;
				} else {
					// Even if class hasn't changed, check if subjects have changed
					$existing_subjects = $wpdb->get_col($wpdb->prepare(
						"SELECT subject_id FROM " . WLSM_STUDENTS_SUBJECTS . " WHERE student_id = %d",
						$student_id
					));

					// Compare existing subjects with new subjects
					$existing_subjects = array_map('intval', $existing_subjects);
					$new_subjects = array_map('intval', $subjects_ids);

					if (array_diff($existing_subjects, $new_subjects) || array_diff($new_subjects, $existing_subjects)) {
						$need_subject_cleanup = true;
					}
				}
			}
			...
			$concession_remarks = '';
			$concession_approved_by = NULL; // NULL instead of 0 to avoid foreign key constraint
			...
			// Suspension Detail.
			$is_suspended      = isset($_POST['is_suspended']) ? (bool) $_POST['is_suspended'] : 0;
			$suspend_start_date = isset($_POST['suspend_start_date']) ? DateTime::createFromFormat(WLSM_Config::date_format(), sanitize_text_field($_POST['suspend_start_date'])) : NULL;
			$suspend_end_date   = isset($_POST['suspend_end_date']) ? DateTime::createFromFormat(WLSM_Config::date_format(), sanitize_text_field($_POST['suspend_end_date'])) : NULL;
			$suspend_message    = isset($_POST['suspend_message']) ? sanitize_text_field($_POST['suspend_message']) : '';
			...
			// }
			if (empty($first_name)) {
				$errors['first_name'] = esc_html__('Please specify student first name.', 'school-management');
			}
			if (empty($last_name)) {
				$errors['last_name'] = esc_html__('Please specify student last name.', 'school-management');
			}
			...
			// Checks if roll number already exists in the section for this session.
			...
			// Guardian Login Detail.
			if ('existing_user' === $guardian_new_or_existing) {
				if (!$guardian_user_id) {
					if (empty($guardian_existing_username)) {
						$errors['guardian_existing_username'] = esc_html__('Please provide existing username.', 'school-management');
					}
				} else {
					if (empty($guardian_new_login_email)) {
						$errors['guardian_new_login_email'] = esc_html__('Please provide login email.', 'school-management');
					}
				}
			} elseif ('new_user' === $guardian_new_or_existing) {
				if (empty($guardian_username)) {
					$errors['guardian_username'] = esc_html__('Please provide username.', 'school-management');
				}
				if (empty($guardian_login_email)) {
					$errors['guardian_login_email'] = esc_html__('Please provide login email.', 'school-management');
				}
				if (!filter_var($guardian_login_email, FILTER_VALIDATE_EMAIL)) {
					$errors['guardian_login_email'] = esc_html__('Please provide a valid email.', 'school-management');
				}
				if (empty($guardian_password)) {
					$errors['guardian_password'] = esc_html__('Please provide login password.', 'school-management');
				}
			}
		...
		if (count($errors) < 1) {
			try {
				...
				if ('existing_user' === $guardian_new_or_existing) {
					if (!$guardian_user_id) {
						// Existing user.
						$guardian_user = get_user_by('login', $guardian_existing_username);
						if (!$guardian_user) {
							throw new Exception(esc_html__('Guardian username does not exist.', 'school-management'));
						}

						$guardian_user_id = $guardian_user->ID;

						// Check if user has a student record.
						// $guardian_user_has_student_record = $wpdb->get_row($wpdb->prepare('SELECT sr.ID FROM ' . WLSM_STUDENT_RECORDS . ' as sr WHERE sr.user_id = %d', $guardian_user_id));

						// if ($guardian_user_has_student_record) {
						// 	throw new Exception(esc_html__('The user already has a student record.', 'school-management'));
						// }
					} else {
						// Update email and password of existing user.
						$guardian_user_data = array(
							'ID'         => $guardian_user_id,
							'user_email' => $guardian_new_login_email,
						);

						if (!empty($guardian_new_password)) {
							$guardian_user_data['user_pass'] = $guardian_new_password;
						}

						$guardian_user_id = wp_update_user($guardian_user_data);
						if (is_wp_error($guardian_user_id)) {
							throw new Exception($guardian_user_id->get_error_message());
						}
					}
				} elseif ('new_user' === $guardian_new_or_existing) {
					// New user.
					$guardian_user_data = array(
						'user_email' => $guardian_login_email,
						'user_login' => $guardian_username,
						'user_pass'  => $guardian_password,
					);

					$guardian_user_id = wp_insert_user($guardian_user_data);
					if (is_wp_error($guardian_user_id)) {
						throw new Exception($guardian_user_id->get_error_message());
					}
				} else {
				...
				$student_record_data = array(
					...
					'suspend_start_date' => $suspend_start_date ? $suspend_start_date->format('Y-m-d') : NULL,
					'suspend_end_date'   => $suspend_end_date ? $suspend_end_date->format('Y-m-d') : NULL,
				...
				if ($auto_roll_number) {
					if ($student_id) {
						...
						// If new admission and auto generate admission number.
						$student_record_data['roll_number'] = WLSM_M_Staff_General::get_roll_number($school_id, $session_id, $section_id);
					}
				}

				if ($student_id) {
					$student_record_data['photo_id']        = isset($student->photo_id) ? $student->photo_id : null;
					$student_record_data['id_proof']        = isset($student->id_proof) ? $student->id_proof : null;
					$student_record_data['parent_id_proof'] = isset($student->parent_id_proof) ? $student->parent_id_proof : null;
					$student_record_data['parent_signature'] = isset($student->parent_signature) ? $student->parent_signature : null;
				}
				...
				if ($student_id) {
					...
					// Delete ALL existing subjects for this student to prevent duplication
					if ($subjects_ids) {
						$wpdb->delete(
							WLSM_STUDENTS_SUBJECTS,
					...
					// Clear existing fee assignments to avoid duplicate rows before re-inserting updated values.
					$wpdb->delete(
						WLSM_STUDENT_FEES,
						array(
							'student_record_id' => $student_id,
				...
				if (count($fee_label)) {
					foreach ($fee_label as $key => $value) {
						...
						// Prevent duplicate fee entries for the same label during update.
						if (isset($processed_labels[$value])) {
							continue;
						}
						$processed_labels[$value] = true;
						...
						if ( isset( $fee_id[ $key ] ) && absint( $fee_id[ $key ] ) ) {
							...
							if ( $selected_fee ) {
								...
								$include_on_promotion = isset( $selected_fee->include_on_promotion ) ? (bool) $selected_fee->include_on_promotion : 1;
							}
						}

						$current_fee_id = $selected_fee ? $selected_fee->ID : NULL;

						// Student fee data.
						$student_fee_data = array(
							'fee_id'               => $current_fee_id,
							'amount'               => isset($fee_amount[$key]) ? $fee_amount[$key] : 0,
							'period'               => isset($fee_period[$key]) ? $fee_period[$key] : '',
							'fee_order'            => $fee_order,
						...
						if ($is_insert) {
							...
							$invoice_data = array(
								...
								'invoice_type'         => ! empty( $student_fee_data['fee_type'] ) ? $student_fee_data['fee_type'] : null,
							);
							...
							if (isset($active_on_dashboard[$key]) && $active_on_dashboard[$key] === '0') {
								$label          = 'Registration';
								$amount         += $student_fee_data['amount'];
							...
							if (isset($active_on_dashboard[$key]) && $active_on_dashboard[$key] === '1') {
							...
							if ($student_fee_exist) {
								...
								$student_fee_data['label']             = $value;
								$student_fee_data['student_record_id'] = $student_id;
								$student_fee_data['fee_type']          = $selected_fee_type;
					...
					if (! empty($route_vehicle)) {
						...
						$transport_invoice_data['fee_type'] = 'transport';

						if (empty($processed_labels[$transport_invoice_data['label']])) {
							$processed_labels[$transport_invoice_data['label']] = true;

							$list_data[] = $transport_invoice_data;
							$amount     += $route_vehicle->fare;
					...
					if (! empty($activity_data)) {
						foreach ($activity_data as $key => $value) {
							...
							if (empty($activity_invoice_data['label']) || isset($processed_labels[$activity_invoice_data['label']])) {
								continue;
							}

							$processed_labels[$activity_invoice_data['label']] = true;
							...
							$list_data[] = $activity_invoice_data;
							$amount     += $value['amount'];
					...
					if (isset($active_on_dashboard[$key]) && $active_on_dashboard[$key] !== "1") {
						$invoice_data['label']                = isset($label) ? $label : 'Registration';
				...
				// Handle transport assignment for EXISTING students only
				if ($student_id && isset($student) && !$is_insert) {
					// Check if transport assignment actually changed
					$old_route_vehicle_id = isset($student->route_vehicle_id) ? $student->route_vehicle_id : null;
					$new_route_vehicle_id = $route_vehicle_id;

					// Only create transport fee if transport assignment changed AND transport is selected
					if ($old_route_vehicle_id != $new_route_vehicle_id && !empty($new_route_vehicle_id)) {
						// Re-fetch route vehicle data to ensure we have current information
						$current_route_vehicle = WLSM_M_Staff_Transport::get_route_vehicle($school_id, $new_route_vehicle_id);

						if ($current_route_vehicle) {
							// Check if transport fee already exists in student fees (may have been inserted by fee_label loop)
							$existing_transport_fee = $wpdb->get_var($wpdb->prepare(
								"SELECT COUNT(*) FROM " . WLSM_STUDENT_FEES . "
								 WHERE student_record_id = %d AND label = 'Transport Fee'",
								$student_id
							));

							// Only create if no transport fee record exists yet
							if ($existing_transport_fee == 0) {
								// Create transport fee record
								$transport_fee_data = array(
									'label'             => 'Transport Fee',
									'amount'            => $current_route_vehicle->fare,
									'period'            => isset($current_route_vehicle->period) ? $current_route_vehicle->period : 'monthly',
									'fee_type'          => 'transport',
									'fee_order'         => 1,
									'student_record_id' => $student_id,
									'created_at'        => current_time('Y-m-d H:i:s'),
								...
								$wpdb->insert(WLSM_STUDENT_FEES, $transport_fee_data);
								...
								$transport_invoice_data = array(
									...
									'transport_month'   => current_time('Y-m'),
									'student_record_id' => $student_id,
									'partial_payment'   => $school_general_auto_invoice_allow_partial_payment ? 1 : 0,
									'added_by'          => $user_id,
									'created_at'        => current_time('Y-m-d H:i:s'),
								);

								$wpdb->insert(WLSM_INVOICES, $transport_invoice_data);
							}
						}
					}
				}
...
?>
		...
		try {
			...
			if (empty($attendance_date)) {
				...
				if ($attendance_date > $current_date) {
					$errors['attendance_date'] = esc_html__('Future dates are not allowed for attendance.', 'school-management');
				}
				$attendance_date = $attendance_date->format('Y-m-d');
			}

			$holiday = WLSM_M_Staff_Class::is_holiday($school_id, $attendance_date);
		} catch (Exception $exception) {
							...
							<div class="col-md-12">
								...
								<?php if ($holiday) : ?>
									<div class="alert alert-info mt-2">
										...
										<?php printf(esc_html__('Today is a holiday: %s. Attendance will be marked as Holiday for all staff.', 'school-management'), esc_html(stripcslashes($holiday->description))); ?>
									</div>
								<?php endif; ?>
							</div>
						</div>
	...
	public static function take_staff_attendance()
	{
		...
		try {
			...
			if (empty($attendance_date)) {
				...
				$disable_future_attendance = get_option('wlsm_disable_future_attendance');
				if ($disable_future_attendance) {
					$current_date = new DateTime();
					if ($attendance_date > $current_date) {
						throw new Exception(esc_html__('Future dates are not allowed for attendance.', 'school-management'));
					}
				}
				$attendance_date = $attendance_date->format('Y-m-d');
			}
	...
	public static function fetch_staff_leaves()
	{
		...
		if (isset($_POST['search']['value'])) {
			...
			if ('' !== $search_value) {
				...
				$search_value_lowercase = strtolower($search_value);
				if (preg_match('/^pending$/', $search_value_lowercase)) {
					$is_approved = 0;
				} elseif (preg_match('/^appr(|o|ove|oved)$/', $search_value_lowercase)) {
					$is_approved = 1;
				} elseif (preg_match('/^reject(|ed)$/', $search_value_lowercase)) {
					$is_approved = 2;
				}
	...
	public static function save_staff_leave()
	{
		...
		try {
			...
			$is_approved   = isset($_POST['is_approved']) ? absint($_POST['is_approved']) : 0;
	...
	public static function save_certificate()
	{
		...
		try {
			...
			$certificate_number_prefix = isset($_POST['certificate_number_prefix']) ? sanitize_text_field($_POST['certificate_number_prefix']) : '';
			$certificate_number_base_input = isset($_POST['certificate_number_base']) ? sanitize_text_field($_POST['certificate_number_base']) : '0';
			$certificate_number_base = absint($certificate_number_base_input);
			$certificate_number_padding = strlen($certificate_number_base_input);
			...
			$fields_to_save['orientation'] = $orientation;
		} catch (Exception $exception) {
			$buffer = ob_get_clean();
			if (!empty($buffer)) {
				$response = $buffer;
		...
		if (count($errors) < 1) {
			try {
				...
				$data = array(
					...
					'certificate_number_base'   => $certificate_number_base,
					'certificate_number_padding' => $certificate_number_padding,
		...
		if (count($errors) < 1) {
			try {
				...
				foreach ($student_ids as $student_id) {
					...
					$certificate_number = WLSM_M_Staff_General::get_certificate_number_by_template($school_id, $certificate_id);
					$serial_number = WLSM_M_Staff_General::get_certificate_serial_number_by_template($school_id, $certificate_id);

					$certificate_student_data['certificate_number'] = $certificate_number;
					$certificate_student_data['serial_number']      = $serial_number;
	...
	public static function send_notification()
	{
		...
		$firebase_to_student = (isset($_POST['firebase_to_student'])) ? (bool) $_POST['firebase_to_student'] : false;
		$firebase_to_parent  = (isset($_POST['firebase_to_parent'])) ? (bool) $_POST['firebase_to_parent'] : false;
				...
				foreach ($student_ids as $student_id) {
					...
					if ($send_firebase) {
						...
						if ($is_enable) {
							...
							// Send to Parent
							if ($firebase_to_parent && $student->parent_user_id) {
								$token = get_option($student->parent_user_id . '_st_notification');
								if ($token) {
									$firebase_success = WLSM_Helper::send_firebase_notification($token, $firebase_title, $firebase_body, $school_id);
									if ($firebase_success) {
										$success = true;
		...
		try {
			...
			foreach ($student_ids_keys as $student_id) {
				if (isset($students[$student_id])) {
					...
					if (count($fees)) {
						$list_data = array();
						$total_grouped_amount = 0;
						$group_invoice_types = array();

						foreach ($fees as $fee) {
							if ( isset( $fee->include_on_promotion ) && ! $fee->include_on_promotion ) {
								continue;
							}

							$fee_order++;
							$normalized_fee_type = WLSM_Helper::normalize_fee_type( isset( $fee->fee_type ) ? $fee->fee_type : '' );
							...
							$student_fee_data = array(
								...
								'include_on_promotion' => (bool) $fee->include_on_promotion,
								'label'                => $fee->label,
								'fee_order'            => $fee_order,
							...
							$fee_data = array(
								...
								'period'          => $student_fee_data['period'],
								'amount'          => $student_fee_data['amount'],
								'partial_payment' => $school_auto_invoice_allow_partial_payment ? 1 : 0,
							);

							$invoice_type    = $normalized_fee_type ? $normalized_fee_type : ( ( stripos( $student_fee_data['label'], 'Transport Fee' ) !== false ) ? 'transport' : null );
							...
							if (isset($fee->active_on_dashboard) && $fee->active_on_dashboard === '1') {
								// Separate individual invoice
								if ($school_generate_invoices_promotion) {
									$success = $wpdb->insert(WLSM_INVOICES, $invoice_data);
								}
							} else {
								// Group this fee into the combined invoice
								$total_grouped_amount += $student_fee_data['amount'];
								if ( $normalized_fee_type ) {
									$group_invoice_types[] = $normalized_fee_type;
								}
								array_push($list_data, $fee_data);
							}
						}
						...
						// Create group invoice with fee type list
						if ($school_generate_invoices_promotion && count($list_data) > 0) {
							$group_invoice_type = null;
							$group_invoice_types = array_values(array_unique(array_filter($group_invoice_types)));
							if (1 === count($group_invoice_types)) {
								$group_invoice_type = $group_invoice_types[0];
							}

							$group_invoice_data = array(
								'label'                => esc_html__('Promotion', 'school-management'),
								'amount'               => $total_grouped_amount,
								'invoice_amount_total' => $total_grouped_amount,
								'discount'             => 0,
								'date_issued'          => current_time('Y-m-d H:i:s'),
								'due_date'             => current_time('Y-m-d H:i:s'),
								'partial_payment'      => $school_auto_invoice_allow_partial_payment ? 1 : 0,
								...
								'student_record_id'    => $new_student_id,
								'created_at'           => current_time('Y-m-d H:i:s'),
								'fee_list'             => serialize($list_data),
								'invoice_type'         => $group_invoice_type
							);
							$wpdb->insert(WLSM_INVOICES, $group_invoice_data);
						}
					}
	...
	public static function fetch_stats_payments()
	{
		...
		// Last 15 Payments
		$payments = WLSM_M_Staff_Accountant::fetch_payments($school_id, $session_id, $now);
	...
	public static function save_school_email_templates_settings()
	{
		...
		try {
			...
			$email_exam_result_report_enable  = isset($_POST['email_exam_result_report_enable']) ? (bool) ($_POST['email_exam_result_report_enable']) : 0;
			$email_exam_result_report_subject = isset($_POST['email_exam_result_report_subject']) ? sanitize_text_field(stripslashes($_POST['email_exam_result_report_subject'])) : '';
			$email_exam_result_report_body    = isset($_POST['email_exam_result_report_body']) ? wp_kses_post(stripslashes($_POST['email_exam_result_report_body'])) : '';
		...
		try {
			...
			// Email Exam Result Report.
			$email_exam_result_report = $wpdb->get_row($wpdb->prepare('SELECT ID, setting_value FROM ' . WLSM_SETTINGS . ' WHERE school_id = %d AND setting_key = "email_exam_result_report"', $school_id));

			$email_exam_result_report_data = array(
				'enable'  => $email_exam_result_report_enable,
				'subject' => $email_exam_result_report_subject,
				'body'    => $email_exam_result_report_body,
			);

			if (!$email_exam_result_report) {
				$wpdb->insert(
					WLSM_SETTINGS,
					array(
						'setting_key'   => 'email_exam_result_report',
						'setting_value' => serialize($email_exam_result_report_data),
						'school_id'     => $school_id,
					)
				);
			} else {
				$wpdb->update(
					WLSM_SETTINGS,
					array('setting_value' => serialize($email_exam_result_report_data)),
					array('ID'            => $email_exam_result_report->ID)
				);
			}
	...
	public static function save_school_sms_carrier_settings()
	{
		...
		try {
			...
			$smslive247_api_key   = isset($_POST['smslive247_api_key']) ? sanitize_text_field($_POST['smslive247_api_key']) : '';
			$smslive247_sender_id = isset($_POST['smslive247_sender_id']) ? sanitize_text_field($_POST['smslive247_sender_id']) : '';
			...
			$pindo_api_token = isset($_POST['pindo_api_token']) ? sanitize_text_field($_POST['pindo_api_token']) : '';
			$pindo_sender    = isset($_POST['pindo_sender']) ? sanitize_text_field($_POST['pindo_sender']) : '';

			$bulksms_sa_username  = isset($_POST['bulksms_sa_username']) ? sanitize_text_field($_POST['bulksms_sa_username']) : '';
			$bulksms_sa_password  = isset($_POST['bulksms_sa_password']) ? sanitize_text_field($_POST['bulksms_sa_password']) : '';
			$bulksms_sa_sender_id = isset($_POST['bulksms_sa_sender_id']) ? sanitize_text_field($_POST['bulksms_sa_sender_id']) : '';
			...
			$africastalking_username = isset($_POST['africastalking_username']) ? sanitize_text_field($_POST['africastalking_username']) : '';
			$africastalking_api_key  = isset($_POST['africastalking_api_key']) ? sanitize_text_field($_POST['africastalking_api_key']) : '';
			$africastalking_sender_id = isset($_POST['africastalking_sender_id']) ? sanitize_text_field($_POST['africastalking_sender_id']) : '';
		...
		if (count($errors) < 1) {
			try {
				...
				// AfricasTalking.
				$africastalking = $wpdb->get_row($wpdb->prepare('SELECT ID, setting_value FROM ' . WLSM_SETTINGS . ' WHERE school_id = %d AND setting_key = "africastalking"', $school_id));

				$africastalking_data = array(
				...
				if (!$africastalking) {
					$wpdb->insert(
						WLSM_SETTINGS,
						array(
							'setting_key'   => 'africastalking',
							'setting_value' => serialize($africastalking_data),
							'school_id'     => $school_id,
						)
					);
				} else {
					$wpdb->update(
						WLSM_SETTINGS,
						array('setting_value' => serialize($africastalking_data)),
						array('ID'            => $africastalking->ID)
					);
				}
				...
				// SMSLive247.
				$smslive247 = $wpdb->get_row($wpdb->prepare('SELECT ID, setting_value FROM ' . WLSM_SETTINGS . ' WHERE school_id = %d AND setting_key = "smslive247"', $school_id));

				$smslive247_data = array(
					'api_key'   => $smslive247_api_key,
					'sender_id' => $smslive247_sender_id,
				);
				...
				if (!$smslive247) {
					$wpdb->insert(
						WLSM_SETTINGS,
						array(
							'setting_key'   => 'smslive247',
							'setting_value' => serialize($smslive247_data),
							'school_id'     => $school_id,
						)
					);
				} else {
					$wpdb->update(
						WLSM_SETTINGS,
						array('setting_value' => serialize($smslive247_data)),
						array('ID'            => $smslive247->ID)
					);
				}
				...
				// bulksms_sa.
				$bulksms_sa = $wpdb->get_row($wpdb->prepare('SELECT ID, setting_value FROM ' . WLSM_SETTINGS . ' WHERE school_id = %d AND setting_key = "bulksms_sa"', $school_id));

				$bulksms_sa_data = array(
					'username'  => $bulksms_sa_username,
					'password'  => $bulksms_sa_password,
					'sender_id' => $bulksms_sa_sender_id,
				);
				...
				if (!$bulksms_sa) {
					$wpdb->insert(
						WLSM_SETTINGS,
						array(
							'setting_key'   => 'bulksms_sa',
							'setting_value' => serialize($bulksms_sa_data),
							'school_id'     => $school_id,
						)
					);
				} else {
					$bulksms_sa_saved_data = unserialize($bulksms_sa->setting_value);

					if (empty($bulksms_sa_data['password']) && isset($bulksms_sa_saved_data['password'])) {
						$bulksms_sa_data['password'] = $bulksms_sa_saved_data['password'];
					}

					$wpdb->update(
						WLSM_SETTINGS,
						array('setting_value' => serialize($bulksms_sa_data)),
						array('ID'            => $bulksms_sa->ID)
					);
				}
				...
				// Pindo SMS settings.
				$pindo_data = array(
					'api_token' => $pindo_api_token,
					'sender'    => $pindo_sender,
				);

				$pindo = $wpdb->get_row($wpdb->prepare('SELECT ID, setting_value FROM ' . WLSM_SETTINGS . ' WHERE school_id = %d AND setting_key = "pindo"', $school_id));
				...
				if (!$pindo) {
					$wpdb->insert(
						...
						array(
							'setting_key'   => 'pindo',
							'setting_value' => serialize($pindo_data),
							'school_id'     => $school_id,
						)
					);
				} else {
					$wpdb->update(
						WLSM_SETTINGS,
						array('setting_value' => serialize($pindo_data)),
						array('ID'            => $pindo->ID)
					);
				}
	...
	public static function save_school_sms_templates_settings()
	{
		...
		try {
			...
			$sms_student_admission_to_student = isset($_POST['sms_student_admission_to_student']) ? (bool) ($_POST['sms_student_admission_to_student']) : 0;
			$sms_student_admission_to_father  = isset($_POST['sms_student_admission_to_father']) ? (bool) ($_POST['sms_student_admission_to_father']) : 0;
			$sms_student_admission_to_mother  = isset($_POST['sms_student_admission_to_mother']) ? (bool) ($_POST['sms_student_admission_to_mother']) : 0;
			...
			$sms_invoice_generated_to_student = isset($_POST['sms_invoice_generated_to_student']) ? (bool) ($_POST['sms_invoice_generated_to_student']) : 0;
			$sms_invoice_generated_to_father  = isset($_POST['sms_invoice_generated_to_father']) ? (bool) ($_POST['sms_invoice_generated_to_father']) : 0;
			$sms_invoice_generated_to_mother  = isset($_POST['sms_invoice_generated_to_mother']) ? (bool) ($_POST['sms_invoice_generated_to_mother']) : 0;
			...
			$sms_online_fee_submission_to_student = isset($_POST['sms_online_fee_submission_to_student']) ? (bool) ($_POST['sms_online_fee_submission_to_student']) : 0;
			$sms_online_fee_submission_to_father  = isset($_POST['sms_online_fee_submission_to_father']) ? (bool) ($_POST['sms_online_fee_submission_to_father']) : 0;
			$sms_online_fee_submission_to_mother  = isset($_POST['sms_online_fee_submission_to_mother']) ? (bool) ($_POST['sms_online_fee_submission_to_mother']) : 0;
			...
			$sms_offline_fee_submission_to_student = isset($_POST['sms_offline_fee_submission_to_student']) ? (bool) ($_POST['sms_offline_fee_submission_to_student']) : 0;
			$sms_offline_fee_submission_to_father  = isset($_POST['sms_offline_fee_submission_to_father']) ? (bool) ($_POST['sms_offline_fee_submission_to_father']) : 0;
			$sms_offline_fee_submission_to_mother  = isset($_POST['sms_offline_fee_submission_to_mother']) ? (bool) ($_POST['sms_offline_fee_submission_to_mother']) : 0;
			...
			$sms_student_admission_to_parent_to_student = isset($_POST['sms_student_admission_to_parent_to_student']) ? (bool) ($_POST['sms_student_admission_to_parent_to_student']) : 0;
			$sms_student_admission_to_parent_to_father  = isset($_POST['sms_student_admission_to_parent_to_father']) ? (bool) ($_POST['sms_student_admission_to_parent_to_father']) : 0;
			$sms_student_admission_to_parent_to_mother  = isset($_POST['sms_student_admission_to_parent_to_mother']) ? (bool) ($_POST['sms_student_admission_to_parent_to_mother']) : 0;
			...
			$sms_invoice_generated_to_parent_to_student = isset($_POST['sms_invoice_generated_to_parent_to_student']) ? (bool) ($_POST['sms_invoice_generated_to_parent_to_student']) : 0;
			$sms_invoice_generated_to_parent_to_father  = isset($_POST['sms_invoice_generated_to_parent_to_father']) ? (bool) ($_POST['sms_invoice_generated_to_parent_to_father']) : 0;
			$sms_invoice_generated_to_parent_to_mother  = isset($_POST['sms_invoice_generated_to_parent_to_mother']) ? (bool) ($_POST['sms_invoice_generated_to_parent_to_mother']) : 0;
			...
			$sms_online_fee_submission_to_parent_to_student = isset($_POST['sms_online_fee_submission_to_parent_to_student']) ? (bool) ($_POST['sms_online_fee_submission_to_parent_to_student']) : 0;
			$sms_online_fee_submission_to_parent_to_father  = isset($_POST['sms_online_fee_submission_to_parent_to_father']) ? (bool) ($_POST['sms_online_fee_submission_to_parent_to_father']) : 0;
			$sms_online_fee_submission_to_parent_to_mother  = isset($_POST['sms_online_fee_submission_to_parent_to_mother']) ? (bool) ($_POST['sms_online_fee_submission_to_parent_to_mother']) : 0;
			...
			$sms_offline_fee_submission_to_parent_to_student = isset($_POST['sms_offline_fee_submission_to_parent_to_student']) ? (bool) ($_POST['sms_offline_fee_submission_to_parent_to_student']) : 0;
			$sms_offline_fee_submission_to_parent_to_father  = isset($_POST['sms_offline_fee_submission_to_parent_to_father']) ? (bool) ($_POST['sms_offline_fee_submission_to_parent_to_father']) : 0;
			$sms_offline_fee_submission_to_parent_to_mother  = isset($_POST['sms_offline_fee_submission_to_parent_to_mother']) ? (bool) ($_POST['sms_offline_fee_submission_to_parent_to_mother']) : 0;
			...
			$sms_absent_student_to_student = isset($_POST['sms_absent_student_to_student']) ? (bool) ($_POST['sms_absent_student_to_student']) : 0;
			$sms_absent_student_to_father  = isset($_POST['sms_absent_student_to_father']) ? (bool) ($_POST['sms_absent_student_to_father']) : 0;
			$sms_absent_student_to_mother  = isset($_POST['sms_absent_student_to_mother']) ? (bool) ($_POST['sms_absent_student_to_mother']) : 0;
			...
			$sms_student_registration_to_student_to_student = isset($_POST['sms_student_registration_to_student_to_student']) ? (bool) ($_POST['sms_student_registration_to_student_to_student']) : 0;
			$sms_student_registration_to_student_to_father  = isset($_POST['sms_student_registration_to_student_to_father']) ? (bool) ($_POST['sms_student_registration_to_student_to_father']) : 0;
			$sms_student_registration_to_student_to_mother  = isset($_POST['sms_student_registration_to_student_to_mother']) ? (bool) ($_POST['sms_student_registration_to_student_to_mother']) : 0;
			...
			$sms_student_invoice_due_date_student_to_student = isset($_POST['sms_student_invoice_due_date_student_to_student']) ? (bool) ($_POST['sms_student_invoice_due_date_student_to_student']) : 0;
			$sms_student_invoice_due_date_student_to_father  = isset($_POST['sms_student_invoice_due_date_student_to_father']) ? (bool) ($_POST['sms_student_invoice_due_date_student_to_father']) : 0;
			$sms_student_invoice_due_date_student_to_mother  = isset($_POST['sms_student_invoice_due_date_student_to_mother']) ? (bool) ($_POST['sms_student_invoice_due_date_student_to_mother']) : 0;
			...
			$sms_student_homework_to_student = isset($_POST['sms_student_homework_to_student']) ? (bool) ($_POST['sms_student_homework_to_student']) : 0;
			$sms_student_homework_to_father  = isset($_POST['sms_student_homework_to_father']) ? (bool) ($_POST['sms_student_homework_to_father']) : 0;
			$sms_student_homework_to_mother  = isset($_POST['sms_student_homework_to_mother']) ? (bool) ($_POST['sms_student_homework_to_mother']) : 0;
			...
			$sms_exam_result_report_enable   = isset($_POST['sms_exam_result_report_enable']) ? (bool) ($_POST['sms_exam_result_report_enable']) : 0;
			$sms_exam_result_report_to_student = isset($_POST['sms_exam_result_report_to_student']) ? (bool) ($_POST['sms_exam_result_report_to_student']) : 0;
			$sms_exam_result_report_to_father  = isset($_POST['sms_exam_result_report_to_father']) ? (bool) ($_POST['sms_exam_result_report_to_father']) : 0;
			$sms_exam_result_report_to_mother  = isset($_POST['sms_exam_result_report_to_mother']) ? (bool) ($_POST['sms_exam_result_report_to_mother']) : 0;

			$sms_exam_result_report_message  = isset($_POST['sms_exam_result_report_message']) ? sanitize_text_field(stripslashes($_POST['sms_exam_result_report_message'])) : '';
			$sms_exam_template_id_result_message = isset($_POST['sms_exam_template_id_result_message']) ? sanitize_text_field(stripslashes($_POST['sms_exam_template_id_result_message'])) : '';

			$exam_result_report_data = array(
				'enable'      => $sms_exam_result_report_enable,
				'to_student'  => $sms_exam_result_report_to_student,
				'to_father'   => $sms_exam_result_report_to_father,
				'to_mother'   => $sms_exam_result_report_to_mother,
				'message'     => $sms_exam_result_report_message,
				'template_id' => $sms_exam_template_id_result_message,
			);
		} catch (Exception $exception) {
			$buffer = ob_get_clean();
			if (!empty($buffer)) {
		...
		try {
			...
			$sms_student_admission_data = array(
				...
				'to_student'  => $sms_student_admission_to_student,
				'to_father'   => $sms_student_admission_to_father,
				'to_mother'   => $sms_student_admission_to_mother,
			...
			$sms_invoice_generated_data = array(
				...
				'to_student'  => $sms_invoice_generated_to_student,
				'to_father'   => $sms_invoice_generated_to_father,
				'to_mother'   => $sms_invoice_generated_to_mother,
			...
			$sms_online_fee_submission_data = array(
				...
				'to_student'  => $sms_online_fee_submission_to_student,
				'to_father'   => $sms_online_fee_submission_to_father,
				'to_mother'   => $sms_online_fee_submission_to_mother,
			...
			$sms_offline_fee_submission_data = array(
				...
				'to_student'  => $sms_offline_fee_submission_to_student,
				'to_father'   => $sms_offline_fee_submission_to_father,
				'to_mother'   => $sms_offline_fee_submission_to_mother,
			...
			$sms_student_admission_to_parent_data = array(
				...
				'to_student'  => $sms_student_admission_to_parent_to_student,
				'to_father'   => $sms_student_admission_to_parent_to_father,
				'to_mother'   => $sms_student_admission_to_parent_to_mother,
			...
			$sms_invoice_generated_to_parent_data = array(
				...
				'to_student'  => $sms_invoice_generated_to_parent_to_student,
				'to_father'   => $sms_invoice_generated_to_parent_to_father,
				'to_mother'   => $sms_invoice_generated_to_parent_to_mother,
			...
			$sms_online_fee_submission_to_parent_data = array(
				...
				'to_student'  => $sms_online_fee_submission_to_parent_to_student,
				'to_father'   => $sms_online_fee_submission_to_parent_to_father,
				'to_mother'   => $sms_online_fee_submission_to_parent_to_mother,
			...
			$sms_offline_fee_submission_to_parent_data = array(
				...
				'to_student'  => $sms_offline_fee_submission_to_parent_to_student,
				'to_father'   => $sms_offline_fee_submission_to_parent_to_father,
				'to_mother'   => $sms_offline_fee_submission_to_parent_to_mother,
			...
			$sms_absent_student_data = array(
				...
				'to_student'  => $sms_absent_student_to_student,
				'to_father'   => $sms_absent_student_to_father,
				'to_mother'   => $sms_absent_student_to_mother,
			...
			$sms_student_registration_to_student_data = array(
				...
				'to_student'  => $sms_student_registration_to_student_to_student,
				'to_father'   => $sms_student_registration_to_student_to_father,
				'to_mother'   => $sms_student_registration_to_student_to_mother,
			...
			$sms_student_invoice_due_date_student_data = array(
				...
				'to_student'  => $sms_student_invoice_due_date_student_to_student,
				'to_father'   => $sms_student_invoice_due_date_student_to_father,
				'to_mother'   => $sms_student_invoice_due_date_student_to_mother,
			...
			$sms_student_homework_data = array(
				...
				'to_student'  => $sms_student_homework_to_student,
				'to_father'   => $sms_student_homework_to_father,
				'to_mother'   => $sms_student_homework_to_mother,
			...
			// SMS Exam Result Report.
			$sms_exam_result_report = $wpdb->get_row($wpdb->prepare('SELECT ID, setting_value FROM ' . WLSM_SETTINGS . ' WHERE school_id = %d AND setting_key = "sms_exam_result_report"', $school_id));
			...
			$sms_exam_result_report_data = array(
				'enable'      => $sms_exam_result_report_enable,
				'to_student'  => $sms_exam_result_report_to_student,
				'to_father'   => $sms_exam_result_report_to_father,
				'to_mother'   => $sms_exam_result_report_to_mother,
			...
			if (!$sms_exam_result_report) {
				$wpdb->insert(
					WLSM_SETTINGS,
					array(
						'setting_key'   => 'sms_exam_result_report',
						'setting_value' => serialize($sms_exam_result_report_data),
						'school_id'     => $school_id,
					)
				);
			} else {
				$wpdb->update(
					WLSM_SETTINGS,
					array('setting_value' => serialize($sms_exam_result_report_data)),
					array('ID'            => $sms_exam_result_report->ID)
				);
			}

			// SMS Academic Report.
			$sms_academic_report = $wpdb->get_row($wpdb->prepare('SELECT ID, setting_value FROM ' . WLSM_SETTINGS . ' WHERE school_id = %d AND setting_key = "sms_academic_report"', $school_id));

			$sms_academic_report_enable   = isset($_POST['sms_academic_report_enable']) ? (bool) ($_POST['sms_academic_report_enable']) : 0;
			$sms_academic_report_to_student = isset($_POST['sms_academic_report_to_student']) ? (bool) ($_POST['sms_academic_report_to_student']) : 0;
			$sms_academic_report_to_father  = isset($_POST['sms_academic_report_to_father']) ? (bool) ($_POST['sms_academic_report_to_father']) : 0;
			$sms_academic_report_to_mother  = isset($_POST['sms_academic_report_to_mother']) ? (bool) ($_POST['sms_academic_report_to_mother']) : 0;
			$sms_academic_report_message  = isset($_POST['sms_academic_report_message']) ? sanitize_text_field(stripslashes($_POST['sms_academic_report_message'])) : '';
			$sms_academic_template_id_report_message = isset($_POST['sms_academic_template_id_report_message']) ? sanitize_text_field(stripslashes($_POST['sms_academic_template_id_report_message'])) : '';

			$academic_report_data = array(
				'enable'      => $sms_academic_report_enable,
				'to_student'  => $sms_academic_report_to_student,
				'to_father'   => $sms_academic_report_to_father,
				'to_mother'   => $sms_academic_report_to_mother,
				'message'     => $sms_academic_report_message,
				'template_id' => $sms_academic_template_id_report_message,
			);

			if (!$sms_academic_report) {
				$wpdb->insert(
					WLSM_SETTINGS,
					array(
						'setting_key'   => 'sms_academic_report',
						'setting_value' => serialize($academic_report_data),
						'school_id'     => $school_id,
					)
				);
			} else {
				$wpdb->update(
					WLSM_SETTINGS,
					array('setting_value' => serialize($academic_report_data)),
					array('ID'            => $sms_academic_report->ID)
				);
			}

			// SMS Academic Multi Group Report.
			$sms_academic_multi_group_report = $wpdb->get_row($wpdb->prepare('SELECT ID, setting_value FROM ' . WLSM_SETTINGS . ' WHERE school_id = %d AND setting_key = "sms_academic_multi_group_report"', $school_id));

			$sms_academic_multi_group_report_enable   = isset($_POST['sms_academic_multi_group_report_enable']) ? (bool) ($_POST['sms_academic_multi_group_report_enable']) : 0;
			$sms_academic_multi_group_report_to_student = isset($_POST['sms_academic_multi_group_report_to_student']) ? (bool) ($_POST['sms_academic_multi_group_report_to_student']) : 0;
			$sms_academic_multi_group_report_to_father  = isset($_POST['sms_academic_multi_group_report_to_father']) ? (bool) ($_POST['sms_academic_multi_group_report_to_father']) : 0;
			$sms_academic_multi_group_report_to_mother  = isset($_POST['sms_academic_multi_group_report_to_mother']) ? (bool) ($_POST['sms_academic_multi_group_report_to_mother']) : 0;
			$sms_academic_multi_group_report_message  = isset($_POST['sms_academic_multi_group_report_message']) ? sanitize_text_field(stripslashes($_POST['sms_academic_multi_group_report_message'])) : '';
			$sms_academic_multi_group_template_id_report_message = isset($_POST['sms_academic_multi_group_template_id_report_message']) ? sanitize_text_field(stripslashes($_POST['sms_academic_multi_group_template_id_report_message'])) : '';

			$academic_multi_group_report_data = array(
				'enable'      => $sms_academic_multi_group_report_enable,
				'to_student'  => $sms_academic_multi_group_report_to_student,
				'to_father'   => $sms_academic_multi_group_report_to_father,
				'to_mother'   => $sms_academic_multi_group_report_to_mother,
				'message'     => $sms_academic_multi_group_report_message,
				'template_id' => $sms_academic_multi_group_template_id_report_message,
			);

			if (!$sms_academic_multi_group_report) {
				$wpdb->insert(
					WLSM_SETTINGS,
					array(
						'setting_key'   => 'sms_academic_multi_group_report',
						'setting_value' => serialize($academic_multi_group_report_data),
						'school_id'     => $school_id,
					)
				);
			} else {
				$wpdb->update(
					WLSM_SETTINGS,
					array('setting_value' => serialize($academic_multi_group_report_data)),
					array('ID'            => $sms_academic_multi_group_report->ID)
				);
			}
	...
	public static function send_test_sms()
	{
		...
		try {
			...
			$template_id = !empty($settings['template_id']) ? $settings['template_id'] : null;
	...
	public static function save_school_registration_settings()
	{
		...
		try {
			...
			$guardian_login    = isset($_POST['registration_guardian_login']) ? (bool) ($_POST['registration_guardian_login']) : '';
			...
			$house             = isset($_POST['registration_house']) ? (bool) ($_POST['registration_house']) : '';
	...
	public static function save_school_dashboard_settings()
	{
		...
		try {
			...
			$school_calendar         = isset($_POST['calendar']) ? (bool)($_POST['calendar']) : '';
			...
			$school_fee_structure       = isset($_POST['fee_structure']) ? (bool)($_POST['fee_structure']) : '';
			$school_tickets             = isset($_POST['tickets']) ? (bool)($_POST['tickets']) : '';
			$school_lessons             = isset($_POST['lessons']) ? (bool)($_POST['lessons']) : '';
			...
			$dashboard_data = array(
				...
				'school_fee_structure'     => $school_fee_structure,
				'school_tickets'           => $school_tickets,
...
<?php
	...
	public static function fetch_id_cards_ajax()
	{
		$current_user = WLSM_M_Role::can('manage_id_cards');
		...
		$output = WLSM_M_Staff_General::get_id_cards($school_id);

		echo wp_json_encode($output);
		die();
	}

	public static function save_id_card_ajax()
	{
		$current_user = WLSM_M_Role::can('manage_id_cards');
		...
		try {
			...
			$id_card_id = isset($_POST['id_card_id']) ? absint($_POST['id_card_id']) : NULL;

			$nonce_action = 'add-id-card';
			if ($id_card_id) {
				$nonce_action = 'edit-id-card-' . $id_card_id;
			}
			...
			$message = WLSM_M_Staff_General::save_id_card($school_id, $_POST, $id_card_id);
	...
	public static function delete_id_card_ajax()
	{
		$current_user = WLSM_M_Role::can('manage_id_cards');
		...
		try {
			...
			$id_card_id = isset($_POST['id_card_id']) ? absint($_POST['id_card_id']) : 0;

			if (! wp_verify_nonce($_POST['delete-id-card-' . $id_card_id], 'delete-id-card-' . $id_card_id)) {
				die();
			}

			$message = WLSM_M_Staff_General::delete_id_card($school_id, $id_card_id);
	...
	public static function fetch_students_printing_ajax()
	{
		$current_user = WLSM_M_Role::can(array('manage_students', 'manage_id_cards'));
		...
		$page     = isset($_POST['draw']) ? absint($_POST['draw']) : 1;
		$length   = isset($_POST['length']) ? absint($_POST['length']) : 10;
		$start    = isset($_POST['start']) ? absint($_POST['start']) : 0;
		$order_by = isset($_POST['order'][0]['column']) ? $_POST['order'][0]['column'] : '';
		$order    = isset($_POST['order'][0]['dir']) ? $_POST['order'][0]['dir'] : 'ASC';
		$search   = isset($_POST['search']['value']) ? sanitize_text_field($_POST['search']['value']) : '';

		$columns = array('sr.ID', 'sr.name', 'sr.admission_number', 'c.label', 'se.label', 'sr.roll_number', 'sr.father_name', 'sr.phone', 'sr.ID');
		$order_by_col = isset($columns[$order_by]) ? $columns[$order_by] : 'sr.name';

		global $wpdb;

		$query = WLSM_M_Staff_General::fetch_students_printing_query($school_id, $session_id, $filter, $search);

		$query_total = WLSM_M_Staff_General::fetch_students_printing_query_count($school_id, $session_id, $filter, $search);

		$query .= " ORDER BY $order_by_col $order LIMIT $length OFFSET $start";
		...
		foreach ($students as $student) {
			$action = '<input type="checkbox" class="wlsm-select-single wlsm-id-card-select" name="student_ids[]" value="' . esc_attr($student->ID) . '">';
			// Removed single print button logic for simplicity as requested feature is bulk print with layout
			// Or I can leave it but it won't use custom layout yet unless I update it
			$print_btn = '<button type="button" class="btn btn-sm btn-outline-success wlsm-print-id-card" data-id-card="' . esc_attr($student->ID) . '" data-nonce="' . esc_attr(wp_create_nonce('print-id-card-' . $student->ID)) . '" data-message-title="' . esc_attr__('Print ID Card', 'school-management') . '" data-message-content="' . esc_attr__('Are you sure?', 'school-management') . '"><i class="fas fa-print"></i></button>';

			$data[] = array(
				$action,
				esc_html(WLSM_M_Staff_Class::get_name_text($student->student_name)),
				esc_html($student->admission_number),
				esc_html(WLSM_M_Class::get_label_text($student->class_label)),
				esc_html(WLSM_M_Staff_Class::get_section_label_text($student->section_label)),
				esc_html(WLSM_M_Staff_Class::get_roll_no_text($student->roll_number)),
				esc_html($student->father_name),
	...
	public static function print_selected_id_cards()
	{
		$current_user = WLSM_M_Role::can(array('manage_students', 'manage_id_cards'));
		...
		if (! wp_verify_nonce($_POST['print-selected-id-cards-nonce'], 'print-selected-id-cards')) {
			die();
		}

		$student_ids = isset($_POST['student_ids']) ? $_POST['student_ids'] : '';
		if (is_string($student_ids) && strpos($student_ids, ',') !== false) {
			$student_ids = explode(',', $student_ids);
		} elseif (is_string($student_ids)) {
			$student_ids = array($student_ids);
		}

		if (empty($student_ids)) {
			wp_send_json_error(esc_html__('No students selected.', 'school-management'));
		}
		...
		$template_id = isset($_POST['id_card_template_id']) ? absint($_POST['id_card_template_id']) : 0;

		$id_card_template = null;
		if ($template_id) {
			$id_card_template = WLSM_M_Staff_General::fetch_id_card($school_id, $template_id);
		}

		// Fetch students
		// We reuse WLSM_M_Staff_General::fetch_students_query but we need to fetch by IDs.
		// Or better, creating a helper to fetch by IDs would be cleaner, but I can just loop them using get_student_record which is heavy.
		// Let's use customized query here for efficiency.
		global $wpdb;
		$placeholders = implode(',', array_fill(0, count($student_ids), '%d'));

		// Ensure placeholders is not empty to avoid SQL error
		if (empty($placeholders)) {
			wp_send_json_error(esc_html__('No valid students selected.', 'school-management'));
		}

		$query = 'SELECT sr.ID, sr.name as student_name, sr.photo_id, sr.admission_number, sr.enrollment_number, sr.roll_number, sr.phone, sr.email, sr.dob, sr.blood_group, sr.address, sr.father_name,
            c.label as class_label, se.label as section_label, ss.label as session_label
            FROM ' . WLSM_STUDENT_RECORDS . ' as sr
            JOIN ' . WLSM_SESSIONS . ' as ss ON ss.ID = sr.session_id
            JOIN ' . WLSM_SECTIONS . ' as se ON se.ID = sr.section_id
            JOIN ' . WLSM_CLASS_SCHOOL . ' as cs ON cs.ID = se.class_school_id
            JOIN ' . WLSM_CLASSES . ' as c ON c.ID = cs.class_id
            WHERE cs.school_id = %d AND sr.session_id = %d AND sr.ID IN (' . $placeholders . ')';

		$params = array_merge(array($school_id, $session_id), $student_ids);
		$students = $wpdb->get_results($wpdb->prepare($query, $params));

		ob_start();

		if ($id_card_template) {
			require_once WLSM_PLUGIN_DIR_PATH . 'admin/inc/school/print/id_cards_custom.php'; // New file
		} else {
			// Fallback to default
			// I'll create id_cards_selected.php which uses default partial
			require_once WLSM_PLUGIN_DIR_PATH . 'admin/inc/school/print/id_cards_selected.php';
		}
