...
if ( ! WLSM_M_Role::can('view_expense_category') ) {
	die();
}
...
<div class="row<?php if ( $expense_category ) { echo ' justify-content-md-center'; } ?> mt-4">
	<?php if ( ! $expense_category || empty( $_GET['id'] ) ) { ?>
	<div class="col-md-6">
			...
			<div>
				<span class="float-md-right">
					<button type="submit" class="btn btn-sm btn-primary" id="wlsm-save-expense-category-btn">
						...
						<?php if ( $expense_category ) { esc_html_e( 'Update Expense Category', 'school-management' ); } else { esc_html_e( 'Add Expense Category', 'school-management' ); } ?>
					</button>
				</span>
			</div>
