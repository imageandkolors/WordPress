...
if ( isset( $_GET['id'] ) && ! empty( $_GET['id'] ) ) {
	...
	if ( $subject ) {
		...
		$exam_paper_count = $wpdb->get_var($wpdb->prepare('SELECT COUNT(ep.ID) FROM ' . WLSM_EXAM_PAPERS . ' as ep JOIN ' . WLSM_EXAMS . ' as ex ON ex.ID = ep.exam_id WHERE ex.school_id = %d AND ep.paper_code = "%s"', $school_id, $subject_code));
		$is_subject_code_in_use = ($exam_paper_count > 0);
	}
}
			...
			<div class="wlsm-form-section">
				<div class="form-row">
					...
					<div class="form-group col-md-6">
						...
						<input type="text" name="code" class="form-control" id="wlsm_code" placeholder="<?php esc_attr_e( 'Enter subject code', 'school-management' ); ?>" value="<?php echo esc_attr( $subject_code ); ?>" <?php if ( ! empty( $is_subject_code_in_use ) ) { echo 'readonly'; } ?>>
						<?php if ( ! empty( $is_subject_code_in_use ) ) { ?>
						<small class="text-danger">
							<?php esc_html_e( 'Subject code cannot be changed as it is already playing a part in exams.', 'school-management' ); ?>
						</small>
						<?php } ?>
					</div>
				</div>
				...
				<div class="form-row">
					...
					<div class="form-group col-md-6">
						...
						<select name="class_id<?php echo isset($id) ? '' : '[]'; ?>"
								class="form-control selectpicker"
								id="wlsm_class_id"
								<?php echo isset($id) ? '' : 'multiple'; ?>
								data-live-search="true"
								data-actions-box="true">
							<?php foreach ($classes as $class) { ?>
								<option <?php selected($class_id, $class->ID, true); ?>
