		...
		if ( count( $errors ) < 1 ) {
			try {
				...
				while ( $line = fgetcsv( $csv_file ) ) {
					...
					$transport_route_name    = isset( $line[27] ) ? sanitize_text_field( $line[27] ) : '';
					$transport_vehicle_number = isset( $line[28] ) ? sanitize_text_field( $line[28] ) : '';
					...
					if ( empty( $class_label ) ) {
						...
						if ( empty( $section_label ) ) {
							...
							if ( $student_exists ) {
								throw new Exception( esc_html__( 'Roll number already exists in this Section.', 'school-management' ) . WLSM_Import::get_csv_error_msg( $row, 6 ) );
							}
					...
					if ( ! empty( $transport_route_name ) && ! empty( $transport_vehicle_number ) ) {
						$route_vehicle = $wpdb->get_row(
							$wpdb->prepare(
								'SELECT rov.ID, ro.fare, ro.period FROM ' . WLSM_ROUTE_VEHICLE . ' AS rov
								JOIN ' . WLSM_ROUTES . ' AS ro ON ro.ID = rov.route_id
								JOIN ' . WLSM_VEHICLES . ' AS v ON v.ID = rov.vehicle_id
								WHERE ro.school_id = %d AND ro.name = %s AND v.vehicle_number = %s
								LIMIT 1',
								$school_id,
						...
						if ( ! $route_vehicle ) {
							throw new Exception(
								esc_html__( 'Transport route/vehicle not found. Check route name and vehicle number.', 'school-management' )
								. WLSM_Import::get_csv_error_msg( $row, 28 )
							);
						}
						$route_vehicle_id = $route_vehicle->ID;
					} elseif ( ! empty( $transport_route_name ) || ! empty( $transport_vehicle_number ) ) {
						// Only one of the two transport fields is provided — require both.
						throw new Exception(
							esc_html__( 'Please provide both Transport Route Name and Vehicle Number, or leave both blank.', 'school-management' )
							. WLSM_Import::get_csv_error_msg( $row, 28 )
						);
					}
					...
					if (count($fees)) {
						foreach ($fees as $key => $value) {
							...
							if ($is_insert) {
								...
								$student_fee_data['fee_id']                = $fees[$key]->ID;
								$student_fee_data['label']                = $fees[$key]->label;
								$student_fee_data['period']               = $fees[$key]->period;
								$student_fee_data['fee_type']             = WLSM_Helper::normalize_fee_type(isset($fees[$key]->fee_type) ? $fees[$key]->fee_type : '');
								$student_fee_data['include_on_promotion'] = isset($fees[$key]->include_on_promotion) ? (bool) $fees[$key]->include_on_promotion : 1;
					...
					// Transport Fee: create student fee and invoice when a route_vehicle was assigned.
					if ( ! empty( $route_vehicle_id ) && isset( $route_vehicle ) && $route_vehicle ) {
						$transport_fee_created_at = current_time( 'Y-m-d H:i:s' );

						$transport_fee_record = array(
							'label'             => 'Transport Fee',
							'amount'            => $route_vehicle->fare,
							'period'            => isset( $route_vehicle->period ) ? $route_vehicle->period : 'monthly',
							'fee_type'          => 'transport',
							'student_record_id' => $student_id,
							'created_at'        => $transport_fee_created_at,
						);
						$wpdb->insert( WLSM_STUDENT_FEES, $transport_fee_record );

						$transport_invoice_number = WLSM_M_Invoice::get_invoice_number( $school_id );
						$transport_invoice = array(
							'label'             => 'Transport Fee',
							'amount'            => $route_vehicle->fare,
							'discount'          => 0,
							'invoice_number'    => $transport_invoice_number,
							'date_issued'       => $transport_fee_created_at,
							'due_date'          => $transport_fee_created_at,
							'partial_payment'   => 0,
							'invoice_type'      => 'transport',
							'transport_month'   => date( 'Y-m' ),
							'student_record_id' => $student_id,
							'added_by'          => get_current_user_id(),
							'created_at'        => $transport_fee_created_at,
						);
						$wpdb->insert( WLSM_INVOICES, $transport_invoice );
					}
		...
		if ( count( $errors ) < 1 ) {
			try {
				...
				while ( $line = fgetcsv( $csv_file ) ) {
					...
					if ( ! $admit_card ) {
						throw new Exception( esc_html__( 'Admit card not found or not published', 'school-management' ) . WLSM_Import::get_csv_error_msg( $row, 1 ) );
					}
