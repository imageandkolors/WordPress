							...
							<table class="table table-bordered table-striped">
								...
								<tbody>
									<?php
										...
										$restrict_to_assigned_subjects = false;
										$admin_subjects = array();

										if( $user_can_manage_subjects && ! $user_can_manage_class ) {
											$restrict_to_assigned_subjects = true;

											// Get admin ID for the current user
											$admin = WLSM_M_Role::get_user_admin( $school_id, $user_id );
											if ( $admin ) {
												global $wpdb;
												$admin_subjects = $wpdb->get_col( $wpdb->prepare( 'SELECT subject_id FROM ' . WLSM_ADMIN_SUBJECT . ' WHERE admin_id = %d', $admin->ID ) );
											}
										}

										foreach ($exam_papers as $key => $exam_paper) {

											if ( $restrict_to_assigned_subjects ) {
												// Check if subject_id is in admin_subjects (loose comparison)
												if ( ! in_array( $exam_paper->subject_id, $admin_subjects ) ) {
													continue;
												}
											}
										...
										?>
											<tr>
												...
												<?php if ($exam_type === 'grade_only') { ?>
												<td>
													<input type="text" name="obtained_grade[<?php echo esc_attr($exam_paper->ID); ?>]" class="form-control" value="<?php echo esc_attr($obtained_grade); ?>">
												</td>
												<?php } else { ?>
