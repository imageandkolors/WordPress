...
<?php if ( ! isset( $bulk_print ) || $bulk_print !== true ) { ?>
<!-- Print result subject-wise. -->
<div class="wlsm-container d-flex mb-2">
	<div class="col-md-12 wlsm-text-center">
		<br>
<?php
$print_css = 'wlsm-result-subject-wise.css';
if ( isset( $is_multi_group_report ) && $is_multi_group_report ) {
	$print_css = 'wlsm-result-subject-wise-landscape.css';
}
?>
		<button type="button" class="<?php echo esc_attr( $print_button_classes ); ?>" id="wlsm-print-result-subject-wise-btn" data-styles='["<?php echo esc_url( WLSM_PLUGIN_URL . 'assets/css/bootstrap.min.css' ); ?>","<?php echo esc_url( WLSM_PLUGIN_URL . 'assets/css/wlsm-school-header.css' ); ?>","<?php echo esc_url( WLSM_PLUGIN_URL . 'assets/css/print/' . $print_css ); ?>"]' data-title="<?php
		printf(
...
<div class="wlsm-container wlsm" id="wlsm-print-results-subject-wise">
	<div class="wlsm-print-results-subject-wise-container">
		...
		<div class="wlsm-heading wlsm-results-subject-wise-heading h5 wlsm-text-center">
			<span>
			<!-- <?php esc_html_e( 'Academic Report ', 'school-management' ); ?> -->
			<?php
			if ($academic_report) {
			...
			<div class="col-md-12">
				<div class="table-responsive w-100">
					<table class="table table-bordered wlsm-results-subject-wise-table wlsm-results-subject-wise-table">
						<?php
						if ( ! isset( $is_multi_group_report ) ) {
							$is_multi_group_report = false;
						}

						if ( $is_multi_group_report ) {
							require WLSM_PLUGIN_DIR_PATH . 'includes/partials/results_subject_wise_multi_group.php';
						} else {
