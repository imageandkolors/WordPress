...
{
	private static function get_invoice_due_amount( $invoice ) {
		$due             = (float) $invoice->payable - (float) $invoice->paid;
		$due_date_amount = 0.0;
		$timezone        = function_exists( 'wp_timezone' ) ? wp_timezone() : null;
		$date_now        = new DateTime( 'now', $timezone );
		$due_date        = ! empty( $invoice->due_date ) ? new DateTime( $invoice->due_date, $timezone ) : null;

		if ( $due_date && $date_now > $due_date ) {
			$due_date_amount = (float) $invoice->due_date_amount;
			$due            += $due_date_amount;
		}

		return array(
			'due'             => WLSM_Config::sanitize_money( $due ),
			'due_date_amount' => WLSM_Config::sanitize_money( $due_date_amount ),
		);
	}
	...
	public static function get_students_with_pending_invoices()
	{
		...
		try {
			...
			if (empty($name) && empty($admission_number)) {
				$errors['student_name'] = esc_html__('Please enter Student Name or Admission Number.', 'school-management');
				wp_send_json_error($errors);
			}
...
?>
	...
	public static function pay_invoice_amount()
	{
		...
		try {
			...
			$invoice_due_data = self::get_invoice_due_amount( $invoice );
			$due              = $invoice_due_data['due'];
			$due_date_amount  = $invoice_due_data['due_date_amount'];

			$invoice_partial_payment = $invoice->partial_payment;

			if ( ! $invoice_partial_payment ) {
				$payment_amount = WLSM_Config::sanitize_money( $due );
			} else {
				if ( ! $payment_amount || $payment_amount <= 0 ) {
					$errors['payment_amount'] = esc_html__( 'Please enter a valid amount.', 'school-management' );
				} elseif ( $payment_amount > $due ) {
		...
		if (count($errors) < 1) {
			...
			try {
				...
				if ('razorpay' === $payment_method) {
					...
					$hash_log_parts = array(
						isset( $payu_fields['key'] ) ? (string) $payu_fields['key'] : '',
						isset( $payu_fields['txnid'] ) ? (string) $payu_fields['txnid'] : '',
						isset( $payu_fields['amount'] ) ? (string) $payu_fields['amount'] : '',
						isset( $payu_fields['productinfo'] ) ? (string) $payu_fields['productinfo'] : '',
						isset( $payu_fields['firstname'] ) ? (string) $payu_fields['firstname'] : '',
						isset( $payu_fields['email'] ) ? (string) $payu_fields['email'] : '',
						isset( $payu_fields['udf1'] ) ? (string) $payu_fields['udf1'] : '',
						isset( $payu_fields['udf2'] ) ? (string) $payu_fields['udf2'] : '',
						isset( $payu_fields['udf3'] ) ? (string) $payu_fields['udf3'] : '',
						isset( $payu_fields['udf4'] ) ? (string) $payu_fields['udf4'] : '',
						isset( $payu_fields['udf5'] ) ? (string) $payu_fields['udf5'] : '',
					);
					$hash_log_string = implode( '|', $hash_log_parts ) . '||||||' . $school_payu_salt;

					error_log( sprintf(
						'[WLSM PayU] Initiating payment | invoice_id=%d | txnid=%s | amount=%s | mode=%s | hash=%s | hash_string=%s | payload=%s',
							$invoice_id,
							$txnid,
							$amount_formatted,
							$school_payu_mode,
				 ...
				 elseif ('pesapal' === $payment_method) {
					...
					if (!empty($invoice_ids)) {
						...
						foreach ($invoice_ids_array as $invoice_id ) {
							...
							$payment_to_record = min($due, $remaining_payment_amount);

							if ($payment_to_record <= 0) {
								continue;
							}
							...
							$pending_payment_data = array(
								...
								'school_id'         => $school_id,
								'attachment'        => $bank_transfer_receipt,
								'created_at'        => current_time('Y-m-d H:i:s')
							);

							$success = $wpdb->insert(WLSM_PENDING_PAYMENTS, $pending_payment_data);
							$remaining_payment_amount -= $payment_to_record;
						}
					} else {
						$receipt_number = WLSM_M_Invoice::get_receipt_number($school_id);
					...
					if (!empty($invoice_ids)) {
						...
						foreach ($invoice_ids_array as $invoice_id ) {
							...
							$pending_payment_data = array(
								...
								'invoice_id'        => $invoice_id,
								'school_id'         => $school_id,
								'attachment'        => $upi_transfer_receipt,
								'created_at'        => current_time('Y-m-d H:i:s')
							);
	...
	public static function process_payu() {
		...
		try {
			...
			$payu_key  = $settings_payu['merchant_key'];
			$payu_salt = $settings_payu['merchant_salt'];
			$payu_mode = $settings_payu['mode'];

			if ( ! WLSM_Payu::verify_hash( $post_data, $payu_key, $payu_salt ) ) {
				WLSM_Payu::log(
					'Callback hash verification failed',
					array(
						'txnid'      => isset( $post_data['txnid'] ) ? sanitize_text_field( $post_data['txnid'] ) : '',
						'posted_hash'=> isset( $post_data['hash'] ) ? $post_data['hash'] : '',
						'status'     => $status,
					)
				);
			...
			$verify_response = WLSM_Payu::verify_payment(
				$txnid,
				$payu_key,
				$payu_salt,
				$payu_mode
			...
			$verified_transaction = array();
			$used_verify_api      = false;

			if ( is_wp_error( $verify_response ) ) {
				WLSM_Payu::log(
					'Verify payment failed, falling back to callback validation',
					array(
						'txnid'   => $txnid,
				...
				WLSM_Payu::log(
					'Parsed verify payment response',
					array(
						'txnid'    => $txnid,
				...
				if ( ! empty( $verify_response['transaction_details'][ $txnid ] ) && is_array( $verify_response['transaction_details'][ $txnid ] ) ) {
					$verified_transaction = $verify_response['transaction_details'][ $txnid ];
					$used_verify_api      = true;
				}
			}
			...
			if ( ! $used_verify_api ) {
				$verified_transaction = array(
					'status'   => isset( $post_data['status'] ) ? sanitize_text_field( $post_data['status'] ) : '',
					'amt'      => $payment_amount,
					'mihpayid' => isset( $post_data['mihpayid'] ) ? sanitize_text_field( $post_data['mihpayid'] ) : '',
				);
			}

			$verified_status = isset( $verified_transaction['status'] ) ? strtolower( sanitize_text_field( $verified_transaction['status'] ) ) : '';
			$verified_amount = isset( $verified_transaction['amt'] )
				? WLSM_Config::sanitize_money( $verified_transaction['amt'] )
				: ( isset( $verified_transaction['amount'] ) ? WLSM_Config::sanitize_money( $verified_transaction['amount'] ) : 0 );

			WLSM_Payu::log(
				'Verification result',
				array(
					'txnid'            => $txnid,
					'used_verify_api'  => $used_verify_api,
					'verified_status'  => $verified_status,
					'verified_amount'  => $verified_amount,
					'payment_amount'   => $payment_amount,
					'verified_payload' => $verified_transaction,
				)
			);

			if ( ! in_array( $verified_status, array( 'success', 'captured' ), true ) || (float) $verified_amount !== (float) $payment_amount ) {
				WLSM_Payu::log(
			...
			$transaction_id = ! empty( $verified_transaction['mihpayid'] )
				? sanitize_text_field( $verified_transaction['mihpayid'] )
				: ( ! empty( $verified_transaction['payuid'] )
					? sanitize_text_field( $verified_transaction['payuid'] )
					: ( ! empty( $post_data['mihpayid'] ) ? sanitize_text_field( $post_data['mihpayid'] ) : $txnid ) );

			$existing_payu_payment = $wpdb->get_var(
				$wpdb->prepare(
					'SELECT ID FROM ' . WLSM_PAYMENTS . ' WHERE payment_method = %s AND transaction_id = %s LIMIT 1',
					'payu',
					$transaction_id
				)
			);

			if ( $existing_payu_payment ) {
				WLSM_Payu::log(
					'Duplicate transaction detected',
					array(
						'txnid'          => $txnid,
						'transaction_id' => $transaction_id,
						'existing_id'    => $existing_payu_payment,
					)
				);
				throw new Exception( esc_html__( 'This PayU transaction has already been processed.', 'school-management' ) );
			}
			...
			$invoice_ids = isset($post_data['udf2']) ? $post_data['udf2'] : '';

			if (!empty($invoice_ids)) {
				$remaining_payment_amount = $payment_amount;
				$invoice_ids_array = array_filter( array_map( 'absint', explode(',', $invoice_ids) ) );
				foreach ($invoice_ids_array as $invoice_id ) {
					if ($remaining_payment_amount <= 0) {
						break;
					}
					$invoice = WLSM_M_Staff_Accountant::get_student_pending_invoice($invoice_id);
					if ( ! $invoice || (int) $invoice->school_id !== (int) $school_id ) {
						throw new Exception( $unexpected_error_message );
					}
					$invoice_due_data = self::get_invoice_due_amount( $invoice );
					$due              = $invoice_due_data['due'];
	...
	public static function get_payu_callback_url() {
		return add_query_arg( 'wlsm_payu_callback', '1', home_url( '/' ) );
	}
				...
				if (WLSM_Paytm::verifychecksum_e($_POST, $paytm_merchant_key, 'TRUE' === $_POST['CHECKSUMHASH'])) {
					...
					if ('01' === $_POST['RESPCODE']) {
						...
						if ('TXN_SUCCESS' === $request_param_list['STATUS'] && $request_param_list['TXNAMOUNT'] === $_POST['TXNAMOUNT']) {
							...
							$invoice_ids = isset($_POST['MERC_UNQ_REF']) ? sanitize_text_field($_POST['MERC_UNQ_REF']) : '';
				...
				if ($sslc->orderValidate($tran_id, $amount, $currency, $_POST)) {
					...
					if( 'VALID' === $_POST['status'] ) {
						...
						if ('VALID' === $request_param_list['status'] && $request_param_list['currency_amount'] === $_POST['currency_amount']) {
							...
							$invoice_ids = isset($_POST['value_a']) ? sanitize_text_field($_POST['value_a']) : '';
