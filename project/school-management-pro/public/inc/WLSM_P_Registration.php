...
class WLSM_P_Registration
{
	public static function submit_registration()
	{
		...
		try {
			...
			$allow_guardian_login = isset($_POST['allow_guardian_login']) ? (bool) ($_POST['allow_guardian_login']) : '';
			...
			$first_names = isset($_POST['first_name_']) && is_array($_POST['first_name_']) ? $_POST['first_name_'] : array();
			$middle_names = isset($_POST['middle_name_']) && is_array($_POST['middle_name_']) ? $_POST['middle_name_'] : array();
			$last_names = isset($_POST['last_name_']) && is_array($_POST['last_name_']) ? $_POST['last_name_'] : array();
			$names       = array(); // Placeholder for count/indices
			...
			$errors = array();
			// Use first_names for iteration count as names is now empty/derived
			foreach ($first_names as $index => $first_name_) {
				$first_name_  = isset($first_names[$index]) ? sanitize_text_field($first_names[$index]) : '';
				$middle_name_ = isset($middle_names[$index]) ? sanitize_text_field($middle_names[$index]) : '';
				$last_name_   = isset($last_names[$index]) ? sanitize_text_field($last_names[$index]) : '';
				$name_        = $first_name_ . ' ' . ($middle_name_ ? $middle_name_ . ' ' : '') . $last_name_;
				...
				if (empty($first_name_)) {
					$errors["first_name_[$index]"] = esc_html__('Please specify first name.', 'school-management');
				}
				if (empty($last_name_)) {
					$errors["last_name_[$index]"] = esc_html__('Please specify last name.', 'school-management');
				}
				// if (empty($dob_)) {
		...
		if (count($errors) < 1) {
			try {
				...
				foreach ($first_names as $index => $first_name_) {
					...
					// Fees.
					$fees = WLSM_M_Staff_Accountant::fetch_fees_by_class($school_id, $class_id, true, $session_id);
				...
				if (count($fees)) {
					foreach ($fees as $fee) {
						...
						$student_fee_data = array(
							...
							'fee_type'             => WLSM_Helper::normalize_fee_type( isset( $fee->fee_type ) ? $fee->fee_type : '' ),
							'include_on_promotion' => isset( $fee->include_on_promotion ) ? (bool) $fee->include_on_promotion : 1,
						);
